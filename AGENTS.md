...
2. 모든 Quilo 저장소 작업은 `npm run harness:init -- <run-id> --objective "<목표>" --tool <codex|claude-code>`로 작업별 체크포인트를 만든다. 단순 질의처럼 저장소를 읽거나 바꾸지 않는 대화만 제외한다.
...
7. 완료 직전에는 이번 작업의 새 교훈이 공통 규칙에 재사용 가능한지 반드시 검토한다. 새 안정 교훈은 `AGENTS.md`, `docs/engineering/agent-harness.md`, `docs/engineering/agent-memory.md`, `.harness/config.json` 또는 관련 계약 테스트에 반영하고 `--promoted-rule`로 기록한다. 새 교훈이 없으면 `--rule-review`에 이유를 남긴다. 이 검토가 없으면 하네스가 `complete`를 거부한다.
...
- `lib/pipelines/chem-pre/generate.js`
  - 화학 사전보고서 Codex 입력 구성, web search, JSON 파싱, 시약·절차 후처리를 수행한다.
...
- `lib/pipelines/chem-result/generate.js`
  - 화학 결과보고서 Codex 입력 구성, 데이터/사진 처리, JSON 파싱, 차트 PNG 렌더링을 수행한다.
...
- `lib/pipelines/phys-result/generate.js`
  ...
  - `.cap`, 엑셀/CSV/텍스트, 매뉴얼 PDF, 이미지, 사용자 메모를 Codex 메시지로 구성한다.
  - Codex JSON 응답 파싱, sanitize, 사용자 메모 과대 반영 방지, 구조화 데이터 보정, 차트 PNG 렌더링을 수행한다.
...
- `lib/pipelines/phys-result/docx-gen.js`
  - Codex가 만든 JSON을 Word `.docx`로 렌더링한다.
...
- `lib/pipelines/phys-result/form.pdf`
  - Codex 입력에 자동 첨부되는 결과보고서 양식 PDF이다.
...
- `lib/anthropic-media.js`
  - 이미지가 Codex 제한을 넘지 않도록 리사이즈/압축하고 vision block으로 변환한다.
...
- `model`: `Codex-opus-4-8`(기본) 또는 `Codex-sonnet-5`. 화학/물리 결과·사전 타입은 GPT(`gpt-5.5`/`gpt-5.4`/`gpt-5.4-mini`)도 허용. 화이트리스트는 `server.js`의 `ALLOWED_MODELS` + `GPT_REPORT_MODELS`.
...
5. `lib/pipelines/phys-result/generate.js`가 Codex 입력을 구성한다.
6. Codex가 JSON 코드 블록 하나를 반환한다.
...
`generate.js`는 사용자 파일을 그대로 Codex에 던지지 않고, 서버에서 최대한 구조화한 뒤 전달한다.
...
- 엑셀/CSV
  - `parseToMarkdown()` 결과를 Codex 입력에 넣는다.
...
- 텍스트 파일
  ...
  - 측정값, 계산 기록, 그래프 해석, 실험 메모인지 Codex가 판단하게 한다.
  - 텍스트에 없는 값은 만들면 안 된다.
- 이미지
  - `prepareImageForAnthropic()`으로 Codex 이미지 제한에 맞게 압축/리사이즈한다.
...
이것은 데이터를 조작하는 기능이 아니다. Codex가 JSON에서 엑셀/CSV 원본과 다른 수치를 쓰거나 서로 충돌하는 서술을 만든 경우, 서버가 업로드 원본에서 파싱한 canonical table을 기준으로 출력 표/서술을 되돌리는 방어 로직이다.
...
- 이미지 크기 제한 문제는 `lib/anthropic-media.js`에서 해결하는 것이 맞다. Codex API 요청 직전에 5MB 제한을 넘기면 안 된다.
...
3. `/api/generate` 입력 검증 오류인지, Codex API 오류인지, JSON 파싱 오류인지, chart 렌더 오류인지, HWPX 빌드 오류인지 나눈다.
