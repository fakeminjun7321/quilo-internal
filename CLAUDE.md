...
- 출력 형식: `.docx`, `.hwpx`
- 현재 허용 모델: `claude-opus-4-8`(기본, 4크레딧), `claude-sonnet-5`(2크레딧). `chem-pre`/`chem-result`/`phys-result`는 GPT(`gpt-5.5` 4 / `gpt-5.4` 1 / `gpt-5.4-mini` 무료)도 선택 가능. 실제 차감 단가는 `lib/pricing.js`의 `MODEL_CREDITS` 기준.
...
- `model`: `claude-opus-4-8`(기본) 또는 `claude-sonnet-5`. 화학/물리 결과·사전 타입은 GPT(`gpt-5.5`/`gpt-5.4`/`gpt-5.4-mini`)도 허용. 화이트리스트는 `server.js`의 `ALLOWED_MODELS` + `GPT_REPORT_MODELS`.
