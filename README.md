# Quilo — 영재고생을 위한 학업 AI

**Quilo**는 영재학교(영재고) 학생의 학업 전반을 돕는 AI입니다. 답을 대신 내주는 범용 AI(ChatGPT)와 달리, 학생이 실제로 한 실험·공부를 **학교에 그대로 낼 수 있는 형태(.hwpx / .docx)로 완성**해 줍니다 — *답은 ChatGPT, 제출은 Quilo.* 영재학교 대구과학고등학교 재학생이 직접 만들었고, **"지어내지 않는다(네가 올린 진짜 자료만 재료로 씀)"**를 원칙으로 합니다.
