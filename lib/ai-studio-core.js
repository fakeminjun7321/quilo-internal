...
function createStudioHandler(spec, deps) {
  ...
  return async function studioHandler(req, res) {
    ...
    try {
      ...
      if (studioReserved > 0) {
        try {
          await supa.refundCredits(userInfo.id, studioReserved);
        } catch (re) {
    ...
    // (레거시/마이그레이션 전이면 예약이 없으므로 기존과 동일한 후불 차감.)
    // 정산 실패는 사용자 결과를 막지 않는다 — 결과는 이미 만들어졌으므로 로그만 남긴다.
    ...
    if (chargeable) {
      ...
      try {
        if (studioReserved > 0) {
          if (cost < studioReserved) {
            const s = await supa.refundCredits(userInfo.id, studioReserved - cost);
            newBalance = s.newBalance;
          } else if (cost > studioReserved) {
            const s = await supa.spendCredits(userInfo.id, cost - studioReserved);
            newBalance = s.newBalance;
          } else {
            newBalance = await supa.getCredits(userInfo.id);
          }
          creditsCharged = cost;
        } else if (cost > 0) {
        ...
        console.error(`[${spec.feature}] settle FAILED (결과는 반환):`, e.message);
      }
    }
