...
function operationFor(entry) {
  ...
  if (["createReport", "estimatePdfTranslation", "createPdfTranslation", "convertDocxToHwpx"].includes(entry.operationId)) {
    ...
    operation.requestBody = {
      ...
      content: {
        "multipart/form-data": {
          schema: {
            type: "object",
            required: isReport ? ["type"] : [isDocx ? "docx" : "pdf"],
            properties: isReport
              ? {
  ...
  if (["downloadJob", "downloadFile", "convertDocxToHwpx"].includes(entry.operationId)) {
    operation.responses[200] = {
      description: "생성 파일",
