...
// owner_id 컬럼 미존재(마이그레이션 전) 판별용.
function isMissingOwnerIdColumn(e) {
  return /owner_id|does not exist|42703|PGRST204|schema cache/i.test(String((e && e.message) || e || ""));
}
...
async function storeUpsert(rec) {
  ...
  if (c && tableOk !== false) {
    try {
      ...
      // owner_id 컬럼이 아직 없으면(마이그레이션 전) 그 컬럼 없이 재시도 — 배포/마이그레이션
      // 순서와 무관하게 동작하도록(하위호환). 그 외 오류는 메모리 fallback.
      if (rec.owner_id !== undefined && isMissingOwnerIdColumn(e)) {
        try {
          const { owner_id, ...legacy } = rec;
          const { error } = await c.from("artifacts").upsert(legacy, { onConflict: "slug" });
          if (error) throw error;
          tableOk = true;
          return rec;
        } catch { tableOk = false; }
...
// 행은 owner_id is null && owner=name 로 별도 조회해 병합한다.
async function storeList({ ownerId, ownerName, isAdmin }) {
  const c = await sb();
  if (c && tableOk !== false) {
    try {
      ...
      const [a, b] = await Promise.all([
        ownerId
          ? c.from("artifacts").select(cols).eq("owner_id", ownerId).order("updated_at", { ascending: false }).limit(300)
          : Promise.resolve({ data: [], error: null }),
        ownerName
          ? c.from("artifacts").select(cols).is("owner_id", null).eq("owner", ownerName).order("updated_at", { ascending: false }).limit(300)
          : Promise.resolve({ data: [], error: null }),
      ]);
  ...
  return [...mem.values()]
    .filter((r) => isAdmin || (ownerId && r.owner_id === ownerId) || ((r.owner_id == null || r.owner_id === "") && r.owner === ownerName))
...
module.exports = function artifactsRouter({
  ...
  // 비교하고, owner_id 가 없는 레거시 행(마이그레이션 전 생성분)에 한해 name 폴백.
  function canManage(req, rec) {
    if (!rec) return false;
    const u = getSessionUser(req) || {};
    if (u.isAdmin) return true;
    if (rec.owner_id != null && rec.owner_id !== "") {
      return !!u.id && rec.owner_id === u.id;
    }
    // 레거시 폴백(owner_id 없음): 예전과 동일하게 표시명으로 판정.
  ...
  const ROUTER_SYS =
    ...
    '   - "eng-exam-prep": 영어 시험대비 3종(모의고사·개념정리·빈칸). Needs English 지문/학습지 → field "source" (1+; pdf/image/text).\n' +
    '   - "korean-lit-exam": 국어(문학) 시험. Needs 학습지/판서 → field "source" (1+; pdf/image); optional 문제은행 → field "bank" (pdf).\n' +
    '   - "cap-translate": PASCO Capstone .cap 화면 텍스트 번역본. Needs exactly one .cap → field "cap".\n' +
    '   - "phys-mock-exam": 물리 모의고사. Needs 기출 시험지 pdf → field "exam" AND 교과서 단원 pdf → field "textbook"; optional 채점기준 pdf → field "rubric".\n' +
    ...
    'Map each provided filename to a field in "fileRoles" for generate. If a generate intent is missing a required file, or it is ambiguous which file is which (e.g. two PDFs for phys-mock-exam and unclear which is 기출 vs 교과서), use "ask".\n' +
  ...
  r.get("/api/artifacts", gate, async (req, res) => {
    try {
      const u = getSessionUser(req) || {};
      res.json({ artifacts: await storeList({ ownerId: u.id || null, ownerName: u.name || "admin", isAdmin: !!u.isAdmin }), persistent: tableOk === true });
    } catch (e) {
