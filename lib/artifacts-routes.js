...
async function callOne(model, userText, wantSummary, projectMode) {
  return isGpt(model)
    ? callGptHtml({ model, userText, wantSummary, projectMode })
    : callClaudeHtml({ model, userText, wantSummary, projectMode });
}
...
async function buildResolved(model, userText, wantSummary, projectMode) {
  if (model && model !== "auto" && BUILD_MODELS.includes(model)) {
    const raw = await callOne(model, userText, wantSummary, projectMode);
  ...
  for (let i = 0; i < ladder.length; i++) {
    const m = ladder[i];
    try {
      const raw = await callOne(m, userText, wantSummary, projectMode);
      lastRaw = raw || lastRaw;
...
async function callClaudeHtml({ model, userText, wantSummary, projectMode }) {
  const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
  const sys = projectMode
    ? PROJECT_SYSTEM
...
async function callGptHtml({ model, userText, wantSummary, projectMode }) {
...
module.exports = function artifactsRouter({ requireAdmin, requireAdminOrBeta, getSessionUser }) {
  ...
  r.post("/api/artifacts/build", gate, async (req, res) => {
    ...
    // 멀티파일 프로젝트 모드: 현재 프로젝트 파일들을 받아 전체를 다시 출력.
    const projectMode = req.body.project === true;
    ...
    try {
      const out = await buildResolved(model, userText, wantSummary, projectMode);
      if (projectMode) {
  ...
  const ROUTER_SYS =
    ...
    'Prefer "chat" when the user is clearly just asking/talking. Use "generate" only when they clearly want a finished document/exam/translation FILE.\n\n' +
    'Reply JSON exactly: {"action":"chat|generate|app|image|ask","reportType":null,"fileRoles":{},"ask":null,"reply":"<one short Korean sentence about what you will do>"}';
  ...
  r.post("/api/studio/route", gate, async (req, res) => {
    ...
    try {
      ...
      if (parsed && typeof parsed === "object") {
        ...
        out.action = ["chat", "generate", "app", "image", "ask"].includes(a)
          ? a
          : "chat";
