...
function quiloDbScript(base, slug) {
  ...
  return `<script>(function(){var A=${JSON.stringify(api)},AI=${JSON.stringify(aiApi)};async function j(u,o){var r=await fetch(u,Object.assign({headers:{'Content-Type':'application/json'}},o));if(!r.ok)throw new Error('KV '+r.status);return r.json();}
window.QuiloDB={all:function(){return j(A);},get:async function(k){return (await j(A))[k];},set:function(k,v){return j(A+'/'+encodeURIComponent(k),{method:'PUT',body:JSON.stringify({value:v})});},push:function(k,item){return j(A+'/'+encodeURIComponent(k)+'/push',{method:'POST',body:JSON.stringify({item:item})});}};
