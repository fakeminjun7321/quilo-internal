...
const { fetchOrGenerate, SEARCH_AVAILABLE, GEN_AVAILABLE } = require("./image-pipeline");
...
## 그림 (figures_needed + 본문 인라인 배치) — 중요

- 정말 보고서 이해에 **꼭 필요한 그림만** 나열. 불필요한 그림은 절대 추가하지 말 것.
- 각 항목의 \`search_query\`는 영어 키워드 4~8개로, 구글 이미지 검색에 바로 쓸 수 있게 작성 (예: "water displacement gas collection apparatus", "CO2 phase diagram critical point").
- 본 실험에 그림이 필요 없다면 \`figures_needed\`는 빈 배열 \`[]\`로 둘 것.

### 그림 본문 배치 (필수)

**모든 그림은 이론 섹션의 \`items\` 배열 안에 인라인으로 배치한다.** 보고서 끝에 별도 그림 목록을 만들지 않는다.
...
최종 출력은 단 하나의 \`\`\`json ... \`\`\` 코드 블록입니다.`,
  ...
  // ── Image acquisition (옵션 ON일 때만) ──
  let imageCost = calcImageCost({ searchCount: 0, generationCount: 0 });
  if (useImages) {
    // 환경변수 상태 진단
    const envStatus = `[진단] 검색 사용 가능: ${SEARCH_AVAILABLE() ? "ON" : "OFF"} (CSE_API_KEY=${process.env.GOOGLE_CSE_API_KEY ? "✓" : "✗"}, CSE_CX=${process.env.GOOGLE_CSE_CX ? "✓" : "✗"}), AI 생성 가능: ${GEN_AVAILABLE() ? "ON" : "OFF"} (GEMINI_API_KEY=${process.env.GEMINI_API_KEY ? "✓" : "✗"})`;
    onProgress(envStatus);

    const figs = parsed.figures_needed;
    if (!Array.isArray(figs) || figs.length === 0) {
      onProgress("· 보고서에 그림 필요 없음 — 이미지 처리 스킵");
    } else if (!SEARCH_AVAILABLE() && !GEN_AVAILABLE()) {
      onProgress(
        "⚠ 이미지 옵션 ON이지만 환경변수 미설정 — 모든 그림 placeholder 유지",
      );
    } else {
      onProgress(`🖼 그림 ${figs.length}장 처리 시작 (검색→AI생성 fallback)`);
      let searchCount = 0;
      let generationCount = 0;
      let attachedCount = 0;
      for (const fig of figs) {
        if (signal?.aborted) {
          onProgress("⚠ abort 신호 감지 — 이미지 처리 중단");
          break;
        }
        const searchTriedBefore =
          SEARCH_AVAILABLE() && (fig.search_query || fig.caption);
        const result = await fetchOrGenerate(fig, { onProgress, signal });
        if (searchTriedBefore) searchCount++;
        if (result) {
          fig._image = {
          ...
          if (result.source === "nano-banana") generationCount++;
          attachedCount++;
        }
      }
      onProgress(
        `🖼 이미지 처리 결과: ${attachedCount}/${figs.length}장 첨부 성공`,
      );
      imageCost = calcImageCost({ searchCount, generationCount });
      const imgLine = formatImageCostLine(imageCost);
      if (imgLine) onProgress(imgLine);
    }
  }
