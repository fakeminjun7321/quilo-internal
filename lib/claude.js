...
최종 출력은 단 하나의 \`\`\`json ... \`\`\` 코드 블록입니다.`,
  ...
  if (useImages && Array.isArray(parsed.figures_needed) && parsed.figures_needed.length > 0) {
    if (!SEARCH_AVAILABLE() && !GEN_AVAILABLE()) {
      onProgress(
        "⚠ 이미지 옵션 ON이지만 GEMINI_API_KEY/GOOGLE_CSE 환경변수가 없습니다. placeholder 유지.",
      );
    } else {
      onProgress(
        `🖼 그림 ${parsed.figures_needed.length}장 처리 시작 (검색→AI생성)`,
      );
      let searchCount = 0;
      let generationCount = 0;
      for (const fig of parsed.figures_needed) {
        if (signal?.aborted) break;
        // Search counts even when we skip the result; we attempted it.
        const searchTriedBefore = SEARCH_AVAILABLE() && (fig.search_query || fig.caption);
        ...
        if (result) {
          ...
          // If search was tried but failed, decrement search count? No — we still pay for search call.
        }
      }
  ...
  } else if (useImages && (!parsed.figures_needed || parsed.figures_needed.length === 0)) {
