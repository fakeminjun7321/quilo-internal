...
최종 출력은 단 하나의 \`\`\`json ... \`\`\` 코드 블록입니다.`,
  ...
  onProgress("Claude Opus가 매뉴얼 분석 중...");

  // Loop: handle web_search tool_use -> tool_result rounds
  ...
  while (attempts < maxAttempts) {
    ...
    // The web_search tool runs server-side; results come back as
    // web_search_tool_result blocks alongside text. We just append the
    // assistant message and continue if stop_reason indicates we should.
    const toolUses = resp.content.filter((b) => b.type === "server_tool_use");
    if (toolUses.length > 0) {
      onProgress(`시약 물성 데이터 검색 중... (${toolUses.length}건)`);
    }
    ...
    // For server-side tool_use (web_search), Anthropic auto-handles the loop;
    // if we got here with a different stop_reason, append assistant turn and
    // continue. (Defensive — typically end_turn is reached in one round.)
  ...
  if (!finalText) {
    throw new Error("Claude가 응답을 완료하지 못했습니다.");
  }
