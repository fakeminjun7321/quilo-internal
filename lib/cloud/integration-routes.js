...
function createCloudIntegrationRouter({ requireAuth, getSessionUser, supa, baseUrl }) {
  ...
  router.get("/google/drive/files/:fileId/download", requireAuth, async (req, res) => {
    try {
      const { accessToken } = await googleContext(req);
      const downloaded = await providers.downloadDriveFile(accessToken, cleanId(req.params.fileId));
      res.set({
  ...
  router.post("/google/drive/files/:fileId/copy", requireAuth, async (req, res) => {
    try {
      ...
      const file = await providers.copyDriveFile(accessToken, cleanId(req.params.fileId), {
  ...
  router.get("/google/drive/files/:fileId/comments", requireAuth, async (req, res) => {
    try {
      const { accessToken } = await googleContext(req);
      res.json({ comments: await providers.listDriveComments(accessToken, cleanId(req.params.fileId), { pageSize: req.query.limit }) });
    } catch (error) { sendCloudError(res, error); }
  });
  ...
  router.post("/google/drive/files/:fileId/comments", requireAuth, async (req, res) => {
    try {
      const { accessToken } = await googleContext(req);
      const comment = await providers.createDriveComment(accessToken, cleanId(req.params.fileId), {
