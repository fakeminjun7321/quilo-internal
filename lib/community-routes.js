...
module.exports = function communityRouter({
  ...
  r.get("/posts", async (req, res) => {
    ...
    try {
      ...
      // feature/suggestion 필터는 기존 링크와 이전 게시글 조회 호환을 위해
      // 읽기에서만 허용한다. 새 글 작성 분류에는 노출하지 않는다.
      const category = comm.normalizeReadablePostCategory(req.query.category);
  ...
  r.post("/posts", requireAuth, async (req, res) => {
    ...
    try {
      ...
      const category = comm.normalizeNewPostCategory(req.body && req.body.category);
      if (!title || !body)
        return res.status(400).json({ error: "제목과 내용을 입력하세요." });
      if (!category) {
        return res.status(400).json({
          error: "자유, 질문, 사용팁, 작업공유 중에서 분류를 선택하세요.",
        });
      }
      ...
      if (e && e.code === "INVALID_COMMUNITY_CATEGORY") {
  ...
  r.post("/appeal", requireAuth, async (req, res) => {
    ...
    try {
      ...
      const kind =
        req.body && ["comment", "generation"].includes(req.body.kind)
          ? req.body.kind
          : "post";
