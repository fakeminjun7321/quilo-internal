...
const RETRY_CONFIDENCE = Math.max(0, Math.min(1, Number(process.env.IMAGE_OCR_RETRY_CONFIDENCE || 0.95)));
...
async function prepareImageForOcr(file, { enhanced = false, variant = "" } = {}) {
  ...
  if (!new Set(["standard", "contrast", "handwriting"]).has(selectedVariant)) {
...
function shouldRetryEnhanced(payload) {
  const confidence = confidenceFromPayload(payload);
  const visibleChars = textFromPayload(payload).replace(/\s|[#*|`_~-]/g, "").length;
  return (confidence.average != null && confidence.average < RETRY_CONFIDENCE)
    || (confidence.minimum != null && confidence.minimum < 0.68)
    || (confidence.lowConfidenceRate != null && confidence.lowConfidenceRate > 0.08)
    || visibleChars < 12;
}
...
async function extractImageText(file, {
  ...
  const normalizedMode = mode === "fast" ? "fast" : mode === "ultra" ? "ultra" : "accurate";
  const candidates = [{ payload: first, prepared }];
  const warnings = [];
  const variants = normalizedMode === "ultra"
    ? ["contrast", "handwriting"]
    : normalizedMode === "accurate" && shouldRetryEnhanced(first)
      ? ["contrast"]
      : [];
  // The two ultra comparison passes are independent. Preparing and requesting
  // them together keeps three-pass accuracy without tripling user wait time.
  ...
  candidates.sort((a, b) => payloadScore(b.payload) - payloadScore(a.payload));
  const selected = candidates[0].payload;
  const selectedPrepared = candidates[0].prepared;
  const comparisons = candidates.slice(1).map((candidate) =>
    textAgreement(textFromPayload(selected), textFromPayload(candidate.payload)));
  ...
  const reviewRequired = (confidence.average != null && confidence.average < 0.94)
    || (confidence.minimum != null && confidence.minimum < 0.62)
    || agreement < 0.82;
  return {
    ...
    quality: {
      ...
      candidateScores: candidates.map((candidate) => ({
        ...
        score: Number(payloadScore(candidate.payload).toFixed(2)),
