...
// 그림 1장 분량의 paragraph 묶음 (이미지 + 캡션 + 출처)
function buildFigureBlock(fig) {
  ...
  if (fig._image && fig._image.buffer) {
    ...
    let attribution = "";
    if (fig._image.source === "nano-banana") {
      attribution = "출처: Google AI Studio로 생성";
    } else if (fig._image.source === "search") {
    ...
    // 이미지 없을 때: 점선 박스 placeholder + 구글 검색 링크
    out.push(...buildFigurePlaceholderBox(fig));
  }
  return out;
}
...
function buildFiguresNeeded(figures = []) {
  ...
  figures.forEach((fig) => {
    ...
    if (fig._image && fig._image.buffer) {
      ...
      // 이미지 없음 — 점선 박스 placeholder + 구글 검색 링크
      out.push(...buildFigurePlaceholderBox(fig));
    }
  });
  return out;
...
async function generateDocx(content) {
  // 이론 본문에서 인라인으로 들어간 그림 번호 추적
  const insertedFigureNumbers = new Set();
  ...
  const sectionChildren = [
    ...
    ...buildTheory(
      content.theory || [],
      content.figures_needed || [],
      insertedFigureNumbers,
    ),
  ...
  // 인라인에 못 들어간 그림이 있으면 끝에 추가 (안전장치)
  const remainingFigures = (content.figures_needed || []).filter(
    (f) => !insertedFigureNumbers.has(Number(f.number)),
  );
  if (remainingFigures.length > 0) {
    sectionChildren.push(...buildFiguresNeeded(remainingFigures));
  }
