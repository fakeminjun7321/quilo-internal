...
const SCHEMA_MISSING_RE =
  /(?:relation|column).*?(?:editorial_|resource_requests|is_staff|is_developer|avatar_url|profile_bio).*?(?:does not exist|not found)|could not find (?:the table|the .* column)|schema cache|PGRST20[45]/i;
...
function dbError(context, error) {
  ...
  if (isSchemaMissingError(error)) {
    return new EditorialError(
      ...
      { code: "EDITORIAL_SCHEMA_MISSING", status: 503, cause: error },
    );
  }
  if (error && error.code === "23505") {
    return new EditorialError("이미 사용 중인 주소(slug)입니다.", {
      code: "EDITORIAL_SLUG_CONFLICT",
      status: 409,
      cause: error,
    });
  }
...
async function listResourceRequests({ manage = false, status = null, limit = 100, offset = 0 } = {}) {
  try {
    const client = storageClient();
