...
function createEditorialRouter({
  ...
  function errorResponse(error, req, res, _next) {
    ...
    if (status >= 500 && code !== "EDITORIAL_SCHEMA_MISSING" && code !== "EDITORIAL_STORAGE_UNAVAILABLE") {
  ...
  async function currentIdentity(req) {
    ...
    try {
      user = await sessionRefresh(req, { failClosed: true }) || user;
    } catch (error) {
  ...
  router.get("/posts", asyncRoute(async (req, res) => {
    const posts = await content.listPublishedPosts({
  ...
  router.get("/resource-requests", asyncRoute(async (req, res) => {
