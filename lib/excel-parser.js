...
function preflightSpreadsheet(buffer, ext) {
  ...
  if (lower === "csv") {
    if (buffer.length > MAX_XLSX_ENTRY_UNCOMPRESSED_BYTES) {
      throw new Error("CSV 파일이 너무 큽니다. 파일을 나눠서 업로드하세요.");
    }
    return;
  }
  if (lower !== "xlsx") return;
  if (buffer.length < 4 || buffer.readUInt32LE(0) !== 0x04034b50) {
    throw new Error("XLSX 파일 구조가 올바르지 않습니다.");
  }
  ...
  if (eocd < 0) throw new Error("XLSX ZIP 중앙 디렉터리를 찾지 못했습니다.");
  ...
  if (
    ...
    throw new Error("XLSX 파일 구조가 비정상적으로 큽니다.");
  }
  ...
  for (let i = 0; i < entryCount; i++) {
    if (off + 46 > buffer.length || buffer.readUInt32LE(off) !== 0x02014b50) {
      throw new Error("XLSX ZIP 항목 구조가 올바르지 않습니다.");
    }
    ...
    if (nameEnd > buffer.length) throw new Error("XLSX ZIP 항목명이 손상되었습니다.");
    if (compressed === 0xffffffff || uncompressed === 0xffffffff) {
      throw new Error("ZIP64 XLSX 파일은 지원하지 않습니다.");
    }
    const name = buffer.toString("utf8", nameStart, nameEnd);
    totalUncompressed += uncompressed;
    if (
      uncompressed > MAX_XLSX_ENTRY_UNCOMPRESSED_BYTES ||
      totalUncompressed > MAX_XLSX_UNCOMPRESSED_BYTES ||
      (/^xl\/worksheets\/.*\.xml$/i.test(name) &&
        uncompressed > MAX_XLSX_SHEET_XML_BYTES) ||
      (/^xl\/sharedStrings\.xml$/i.test(name) &&
        uncompressed > MAX_XLSX_SHARED_STRINGS_BYTES)
    ) {
      throw new Error("XLSX 파일이 너무 큽니다. 시트/문자열 수를 줄여 업로드하세요.");
    }
...
function parseToMarkdown(buffer, ext) {
  let workbook;
  try {
    preflightSpreadsheet(buffer, ext);
    if (ext === "csv") {
...
module.exports = { parseToMarkdown, parseToTables, preflightSpreadsheet };
