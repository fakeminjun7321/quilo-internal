...
const DEFAULT_SCOPES = Object.freeze([
  ...
  "integrations:read", "integrations:write", "webhooks:read", "webhooks:write",
]);
...
function safeRedirectUris(values) {
  ...
  const extra = String(process.env.MCP_OAUTH_REDIRECT_ALLOWLIST || "").split(",").map((v) => v.trim()).filter(Boolean);
  return [...new Set(values.map(String))].filter((value) => {
    try {
      const url = new URL(value);
      if (url.protocol !== "https:") return process.env.NODE_ENV !== "production" && ["localhost", "127.0.0.1"].includes(url.hostname);
      return url.hostname === "chatgpt.com" || url.hostname.endsWith(".chatgpt.com") || extra.some((prefix) => value.startsWith(prefix));
    } catch { return false; }
  });
}
...
function createMcpOAuthRouter({ supa, getSessionUser, baseUrl }) {
  ...
  router.get("/oauth/authorize", async (req, res) => {
    ...
    if (!scopes.length || requested.some((scope) => !ALLOWED_SCOPES.includes(scope))) return res.status(400).send("지원하지 않는 OAuth scope입니다.");
