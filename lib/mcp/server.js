...
const TOOLS = Object.freeze([
  ...
  tool({ name: "google_drive_files", title: "Google Drive 파일", description: "Use this when Quilo가 접근할 수 있는 사용자의 Google Drive 파일을 찾아야 할 때 사용합니다.", scopes: ["integrations:read"], inputSchema: { properties: { query: { type: "string" }, limit: { type: "integer", minimum: 1, maximum: 100 } } } }),
  ...
  tool({ name: "google_drive_comments", title: "Google Drive 댓글", description: "Use this when Google Drive 파일에 달린 댓글과 답글을 읽어야 할 때 사용합니다.", scopes: ["integrations:read"], inputSchema: { properties: { fileId: { type: "string" } }, required: ["fileId"] } }),
