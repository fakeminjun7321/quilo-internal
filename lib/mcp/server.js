...
const TOOLS = Object.freeze([
  ...
  tool({ name: "community_posts", title: "커뮤니티 글", description: "Use this when Quilo 커뮤니티의 건의·기능 요청 글을 읽어야 할 때 사용합니다.", scopes: ["community:read"], inputSchema: { properties: { category: { type: "string" } } } }),
...
function createMcpRouter({ baseUrl, supa }) {
  ...
  router.post("/", async (req, res) => {
    ...
    for (const message of messages) {
      ...
      responses.push(await handleMessage(req, message, baseUrl, supa));
    }
...
async function handleMessage(req, message, baseUrl, supa) {
  if (message.jsonrpc !== "2.0" || typeof message.method !== "string") return rpcError(message.id ?? null, -32600, "Invalid Request");
  if (message.method === "initialize") return rpcResult(message.id, { protocolVersion: supportedProtocol(message.params?.protocolVersion), capabilities: { tools: { listChanged: false } }, serverInfo: { name: "Quilo", title: "Quilo 학업 AI", version: "1.0.0" }, instructions: "Quilo의 전체 기능은 search/fetch로 찾고, 사용자별 읽기·쓰기 기능은 OAuth 연결 후 사용하세요." });
  ...
  try {
    ...
    const data = await callTool(req, spec.name, message.params?.arguments || {}, baseUrl);
...
async function callTool(req, name, args, baseUrl) {
  if (name === "search") {
    const features = listFeatures({ query: String(args.query || "").slice(0, 200) }).slice(0, 20);
  ...
  if (name === "fetch") {
    const feature = getFeature(String(args.id || ""));
