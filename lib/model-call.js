...
// Gemini·Kimi 는 OpenAI 호환 엔드포인트를 제공하므로 base/key/파라미터 프로필만 다르게
// 주면 같은 경로로 호출된다. ⚠ Kimi(Moonshot)는 중국 서버(foreign:true) — 전송 직전
// 개인정보 스크럽 대상(foreign-scrub).
const PROVIDERS = {
  openai: {
    label: "GPT",
    base: () => GPT_BASE,
  ...
  kimi: {
    label: "Kimi",
    base: () => process.env.MOONSHOT_API_BASE || "https://api.moonshot.ai/v1",
    key: () => process.env.MOONSHOT_API_KEY || process.env.KIMI_API_KEY || "",
    tokenParam: "max_tokens",
    reasoning: false,
    foreign: true, // ⚠ 중국 서버(Moonshot AI, 베이징)
  },
};
...
function providerFor(model) {
  ...
  if (/^kimi/.test(m) || /^moonshot/.test(m)) return "kimi";
  return null; // Claude 등은 각 파이프라인의 Claude 경로가 처리
}
// Claude 가 아닌 OpenAI 호환 제공자(gpt/gemini/kimi)인가 — 파이프라인 라우팅용.
...
async function callGptReport({
  ...
  if (r.finish === "length") {
    ...
    // Gemini/Kimi 등 비추론 제공자는 effort 를 보내지 않고 예산만 키운다.
