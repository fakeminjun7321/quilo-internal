...
// Gemini·Qwen 은 OpenAI 호환 엔드포인트를 제공하므로 base/key/파라미터 프로필만 다르게
// 주면 같은 경로로 호출된다. ⚠ Qwen(DashScope)은 중국 서버(foreign:true) — 전송 직전
...
const PROVIDERS = {
  ...
  qwen: {
    ...
    key: () => process.env.DASHSCOPE_API_KEY || process.env.QWEN_API_KEY || "",
...
// Claude 가 아닌 OpenAI 호환 제공자(gpt/gemini/qwen)인가 — 파이프라인 라우팅용.
...
async function callGptReport({
  ...
  if (r.finish === "length") {
    ...
    // Gemini/Qwen 등 비추론 제공자는 effort 를 보내지 않고 예산만 키운다.
