...
4. **사람이 읽는 모든 텍스트는 한국어**로: 보고서 제목, 표 헤더(data_table.headers), 실험명(experiments[].name), 캡션, 본문, 분석. 영어 ASCII 는 **차트 이미지 안 텍스트**(chart 의 title/x_label/y_label/series.label/trendline.label)에만 허용. 표·제목·실험명을 영어로 두지 말 것.
...
async function callGptReport({
  ...
  const body = {
    model,
    messages,
    // 추론+출력이 한 예산을 공유하므로 넉넉히. (이전엔 32000 으로 하향 캡 →
    // 추론이 예산을 먹으면 JSON 이 잘렸다.)
    max_completion_tokens: Math.max(maxTokens, 32000),
    // 추론 토큰 과소비로 인한 출력 잘림 방지(보고서는 깊은 추론 불필요).
