...
async function callGptReport({
  ...
  async function once(maxTok, effort) {
    ...
    for (let attempt = 1; ; attempt++) {
      let resp;
      try {
        resp = await fetch(`${GPT_BASE}/chat/completions`, {
          method: "POST",
          headers: {
      ...
      if (RETRYABLE.has(status) && attempt < MAX_ATTEMPTS && !signal?.aborted) {
        ...
        onProgress(
          `⚠️ OpenAI ${status} — ${Math.round(waitMs / 1000)}초 후 재시도 (${attempt}/${MAX_ATTEMPTS - 1})`,
        );
        await sleep(waitMs);
        continue;
      }
    ...
    if (status < 200 || status >= 300) {
      throw new Error(`OpenAI ${status}: ${String(raw).slice(0, 300)}`);
    }
    let j;
    try {
      j = JSON.parse(raw);
      ...
      throw new Error(
        `OpenAI 응답을 해석할 수 없습니다(${status}, ${raw.length}바이트)${raw ? ": " + raw.slice(0, 160) : " — 빈 응답"}`,
      );
    }
