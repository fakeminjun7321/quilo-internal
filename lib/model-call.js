...
const foreignScrub = require("./foreign-scrub");
...
// ── 제공자 프로필 ─────────────────────────────────────────────────────────────
// 보고서 생성은 Anthropic content 블록을 OpenAI chat/completions 형식으로 바꿔 호출한다.
// Gemini 는 OpenAI 호환 엔드포인트를 제공하므로 base/key/파라미터 프로필만 다르게
// 주면 같은 경로로 호출된다. (foreign:true 제공자는 전송 직전 개인정보 스크럽 대상 —
// 현재 활성 해외 제공자는 없음. 향후 중국 등 제공자 추가 시 프로필에 foreign:true 만 주면 된다.)
const PROVIDERS = {
  openai: {
    label: "GPT",
    base: () => GPT_BASE,
    ...
    reasoning: true, // reasoning_effort 지원(gpt-5.x)
    foreign: false,
  },
  gemini: {
    label: "Gemini",
    ...
    tokenParam: "max_tokens", // Gemini OpenAI 호환은 max_tokens
    reasoning: false, // reasoning_effort 미지원 — 보내면 거부될 수 있음
    foreign: false,
  },
};
...
function providerFor(model) {
  ...
  return null; // Claude 등은 각 파이프라인의 Claude 경로가 처리
}
// Claude 가 아닌 OpenAI 호환 제공자(gpt/gemini)인가 — 파이프라인 라우팅용.
function usesOpenAiCompat(model) {
  return providerFor(model) !== null;
}
// 중국 등 해외 서버 제공자인가 — 개인정보 스크럽·동의 게이트 대상.
function isForeignModel(model) {
  const id = providerFor(model);
  return !!(id && PROVIDERS[id].foreign);
}
// 해당 모델의 제공자 키가 설정돼 있는가(BYOK 포함).
function providerConfigured(model) {
  const id = providerFor(model);
  return !!(id && PROVIDERS[id].key());
}
function providerLabel(model) {
  const id = providerFor(model);
  return id ? PROVIDERS[id].label : "Claude";
}
...
// 429/5xx 재시도 백오프: 0.8s, 1.6s, 3.2s ... 최대 15s + 지터.
function gptBackoffMs(attempt) {
...
2. 스키마의 **모든 필드를 빠짐없이 채울 것**. 객체를 빈 {} 나 빈 "" 로 두지 말 것(예: pcei, conclusion, analysis 는 비우면 안 됨). 필수 섹션은 빈 배열 [] 로도 반환하지 말 것. 내용 생성이 어려우면 해당 필드에 최소 1개의 실질 항목이라도 채울 것(위 지침이 특정 모드에서 빈 값을 명시적으로 요구하는 필드만 예외).
...
8. **긴 하이픈/대시(— – ―) 문장부호를 절대 쓰지 말 것.** 연결·부연은 쉼표나 괄호, '즉' 같은 접속 표현으로 쓰고, 숫자 범위는 '~'를 쓸 것.
9. 본문·표·캡션에 생성 과정 내부 용어(서버, 코드, 프롬프트, 주입, AI 등)를 노출하지 말 것. "서버 계산값", "AI가 산출한 값" 같은 표현 금지. 모든 수치·계산은 학생이 직접 수행한 것처럼 서술할 것.
10. 차트 spec 에는 반드시 실제 데이터 포인트를 포함할 것(series 의 숫자 values 배열 또는 points 의 {x, y} 객체). 라벨만 있고 데이터가 없는 차트나 빈 series 를 만들지 말 것.
──────────────────────────────────────────────`;

// BYOK 컨텍스트(사용자 등록 키) 우선, 없으면 서버 env 키.
function gptKey() {
  return byok.openaiKey();
}
...
async function callGptReport({
  ...
  signal,
  onProgress = () => {},
  redact = null, // 해외(중국) 제공자 전송 시 제거할 정확한 개인정보 값(이름·학번 등)
}) {
  const providerId = providerFor(model) || "openai";
  const prof = PROVIDERS[providerId];
  const key = prof.key();
  if (!key) {
    throw new Error(`${prof.label} API 키가 설정되지 않았습니다.`);
  }
  // 해외(중국) 제공자면 전송 직전 개인정보 스크럽. 측정값은 절대 건드리지 않는다.
  let sysForCall = system;
  let contentForCall = content;
  if (prof.foreign) {
    const terms = Array.isArray(redact) ? redact : [];
    sysForCall = foreignScrub.scrubSystem(system, terms);
    contentForCall = foreignScrub.scrubBlocks(content, terms);
  }
  const base = prof.base();
  const messages = [
    { role: "system", content: systemToText(sysForCall) + GPT_SYSTEM_ADDENDUM },
    { role: "user", content: toOpenAiContent(contentForCall) },
  ];
  // 1회 호출. 추론+출력이 한 예산을 공유하므로 넉넉히 잡는다.
  async function once(maxTok, effort) {
    const body = { model, messages };
    body[prof.tokenParam] = maxTok;
    if (prof.reasoning && effort) body.reasoning_effort = effort;
    if (jsonObject) body.response_format = { type: "json_object" };
    // 일시적 오류(429 레이트리밋 / 5xx)는 지수 백오프로 재시도.
    // 문제집 메이커처럼 GPT 를 병렬로 fan-out 하는 경우 429 가 잘 나므로 필요.
    ...
    for (let attempt = 1; ; attempt++) {
      ...
      try {
        ...
        await sleep(gptBackoffMs(attempt));
        continue;
      }
      status = resp.status;
      if (RETRYABLE.has(status) && attempt < MAX_ATTEMPTS && !signal?.aborted) {
        const ra = Number(resp.headers.get("retry-after"));
        const waitMs = Number.isFinite(ra) && ra > 0 ? ra * 1000 : gptBackoffMs(attempt);
        onProgress(
          `⚠️ ${prof.label} ${status} — ${Math.round(waitMs / 1000)}초 후 재시도 (${attempt}/${MAX_ATTEMPTS - 1})`,
        );
        await sleep(waitMs);
        continue;
      }
    ...
    if (status < 200 || status >= 300) {
      throw new Error(`${prof.label} ${status}: ${String(raw).slice(0, 300)}`);
    }
    let j;
    try {
      j = JSON.parse(raw);
      ...
      throw new Error(
        `${prof.label} 응답을 해석할 수 없습니다(${status}, ${raw.length}바이트)${raw ? ": " + raw.slice(0, 160) : " — 빈 응답"}`,
      );
    }
  ...
  let r = await once(cap1, prof.reasoning ? GPT_REASONING_EFFORT : null);
  if (r.finish === "length") {
    onProgress("⚠️ 출력이 길어 더 큰 예산으로 1회 재시도합니다...");
    // 추론 끄고(none) 출력 예산을 최대로 — gpt-5.x는 'minimal'을 지원하지 않음(none/low/medium/high/xhigh).
    // Gemini 등 비추론 제공자는 effort 를 보내지 않고 예산만 키운다.
    r = await once(Math.max(cap1 * 2, 96000), prof.reasoning ? "none" : null);
  }
  if (r.finish === "length") {
    throw new Error(
      `${prof.label} 출력이 최대 길이에 도달해 잘렸습니다(재시도 후에도). 입력을 줄이거나 다시 시도하세요.`,
    );
  }
  if (!r.text || !r.text.trim()) {
    throw new Error(`${prof.label}가 빈 응답을 반환했습니다.`);
  }
  const u = r.usage;
