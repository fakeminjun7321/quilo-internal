...
// OUTPUT_VALIDATE_ENFORCE=1 일 때 생성 실패로 승격되는 규칙 집합.
// raw 수식 마커만 대상이다: 마커 잔존은 수식 postprocess 가 사실상 실패한 상태라
// "HWPX 수식 postprocess 실패는 fatal" 원칙과 같은 선상에서 다룬다.
// 긴 대시, U+FFFD, BinData 누락은 상류 스크럽(stripAiDashes 등)과 생성기 후처리가
// 1차 방어이고 템플릿 유래 문자가 오탐될 수 있어 enforce 모드에서도 경고로만 남긴다.
const ENFORCEABLE_RULES = new Set(["raw-marker"]);
...
// 기존 magic-byte 검사의 하위호환 계약은 그대로 유지한다. 새 단일 PDF 산출물만
// 별도의 엄격한 postflight를 적용해 전송 중 잘린 파일을 차단한다.
...
// 생성기가 돌려준 filename/MIME은 신뢰하지 않고 실제 output kind에 맞춰 고정한다.
...
async function validateReportArtifact(value, opts = {}) {
  ...
  try {
    ...
    // 계약: 이 함수는 어떤 경우에도 throw 하지 않는다. 검사기 자체 오류도 문제
    // 목록으로만 보고한다(ENFORCEABLE_RULES 밖이므로 생성 실패로는 이어지지 않음).
    problems.push({
