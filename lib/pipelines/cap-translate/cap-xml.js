...
// 정규식으로 캡처그룹 1(속성값)의 시작/끝 인덱스를 함께 돌려주는 헬퍼.
function* matchAllAttr(str, regex, groupIndex) {
  let m;
  regex.lastIndex = 0;
  while ((m = regex.exec(str)) !== null) {
    const value = m[groupIndex];
    // 캡처그룹의 절대 위치 계산: 전체 매치 안에서 value 의 첫 등장 오프셋.
    const matchStart = m.index;
    const rel = m[0].indexOf(value);
    const valueStart = matchStart + (rel >= 0 ? rel : 0);
    yield { value, valueStart, valueEnd: valueStart + value.length };
    if (m.index === regex.lastIndex) regex.lastIndex++; // 빈 매치 무한루프 방지
  }
}
