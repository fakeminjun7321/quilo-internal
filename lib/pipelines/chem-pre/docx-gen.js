...
function buildApparatusAndChemicals(content) {
  ...
  (content.chemicals || []).forEach((ch, idx) => {
    ...
    const refMarker = sourceUrl && refIndex.has(sourceUrl)
      ...
      : "";
    const head = `(${idx + 1}) **${ch.name || ""}** (${ch.iupac || ""}, ${ch.formula || ""})${refMarker}`;
    out.push(
      p(head, {
        size: 22,
        indent: { left: convertMillimetersToTwip(5) },
...
function buildMinimalApparatus(content) {
  ...
  apps.forEach((a) => {
    const enName = a.name_en ? ` (${a.name_en})` : "";
    const detail = a.description ? `: ${a.description}` : "";
    blocks.push(p(`${a.name || ""}${enName}${detail}`, { indent: { left: 240 } }));
  });
  ...
  if ((content.chemicals_summary_table || []).length > 0) {
    blocks.push(emptyP());
    blocks.push(p("[표 1] 시약 요약", { bold: true, spaceAfter: 60 }));
    blocks.push(buildChemicalsSummaryTable(content.chemicals_summary_table));
  }
