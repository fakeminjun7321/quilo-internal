...
"items": [
  ...
  "{{EQ:PV = nRT}}",
  "분자량은 측정값으로부터 다음과 같이 계산한다:",
  "{{EQ:MW = {mRT} over {PV}}}",
  "공기의 주성분 산화 반응식: {{EQ:N_2 + O_2 -> 2NO}}"
]
\`\`\`

**언제 어떤 마커를 쓸지**:
- 본문 변수 한두 개 (\`*PV* = *nRT*\`) → 인라인 마커 (\`*\`, \`_{}\`, \`^{}\`)
- 별도 줄에 보여줄 수식, 분수·적분·행렬·반응식 → \`{{EQ:...}}\`
- 본문 다른 곳에서 "식 (1)에서 본 바와 같이"처럼 참조할 중요 수식 → \`{{EQN:...}}\` (자동으로 (1), (2) 번호 부여)
- 둘을 섞지 마세요. \`{{EQ:...}}\` 안에서는 인라인 마커(\`*\`, \`**\`)를 쓰지 않습니다.
...
"items": [
  "이상기체 상태방정식은 다음과 같다:",
  "{{EQN:PV = nRT}}",
  "식 (1)을 변형하면 분자량을 다음 식으로 구할 수 있다:",
  "{{EQN:MW = {mRT} over {PV}}}",
  "위 식 (2)에 측정값을 대입하면 공기의 분자량이 나온다."
]
\`\`\`

→ 한글에서 자동으로 \`(1)\`, \`(2)\` 번호가 식 옆에 붙는다.
