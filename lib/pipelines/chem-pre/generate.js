...
매뉴얼에 Data, Discussion, Conclusion, Questions, 고찰 항목이 있으면 \`data_table\`과 \`expected_discussion\`에 반드시 반영한다.`,
  minimal: `## 현재 스타일 모드

**STYLE_MODE: minimal** (필요한 내용만 적기 — 잘 만든 학생 사전보고서 스타일)

위 스킬 명세의 \`minimal\` 모드 섹션을 정확히 따르라. 핵심 요약:
- 표지·시약 요약 표·그림 placeholder 없음. 분량 4~7페이지.
- 가. 나. 다. 헤더 사용 금지. (1) (2) (3) 또는 ① ② ③만.
- 이론적 배경: 핵심 용어 5~7개를 (1)~(5) 하위번호로 1단락씩 정의 (사례·역사·복잡한 수식 도출 X).
- 실험 준비물: 한 문단 콤마 나열. 시약 물성 데이터(\`molar_mass\`, \`mp_bp\`, \`density\`, \`properties\`, \`toxicity\`)는 비워도 됨.
- \`chemicals_summary_table\`: \`[]\`, \`figures_needed\`: \`[]\`.
- \`references\`: 1~3개 (없으면 \`[]\`).
...
- 반응식·계산식은 일반 문장 안에서 \`H_{2}O\`, \`→\`, \`×\`, \`/\` 등을 사용해 자연스럽게 표기하고, \`{{EQ:...}}\` 같은 중괄호 수식 마커는 절대 쓰지 마세요.`;
...
2. Theory & Principle 키워드 각각에 대해 대구과학고 2학년 일반화학 수준의 이론을 전개합니다 (수식·반응식 포함).
...
\`\`\`json
{
  ...
  "apparatus": [
    {
      ...
      "description": "한 문장, 35자 안팎의 핵심 용도"
    }
  ],
  "chemicals": [
    {
  ...
  "data_table": [
    { "item": "한계 시약", "value": "Salicylic acid" },
    { "item": "Salicylic acid 몰수", "value": "2.000 g / 138.12 g·mol^{-1} = 0.01448 mol" },
    { "item": "이론적 수득량", "value": "0.01448 mol × 180.16 g·mol^{-1} = 2.609 g" },
    { "item": "실험 수득량", "value": "실험 후 측정" },
    { "item": "수득률", "value": "실험 수득량 / 2.609 g × 100%" }
  ],
  "expected_discussion": [
    {
      "question": "한계 시약과 이론적 수득량 및 수득률",
      "answer": "Salicylic acid가 한계 시약이므로 aspirin의 이론적 수득량은 2.609 g이고, 실험 후 측정 질량을 이 값으로 나누어 수득률을 계산한다."
    }
  ],
  "figures_needed": [
    {
...
## 매뉴얼 Data/Discussion 반영 (감점 방지)

- 매뉴얼에 Data 표, 빈 계산표, Discussion/Conclusion/Questions 체크리스트가 있으면 절대 무시하지 마세요.
- 사전보고서라도 Data 표에 들어갈 **문헌값·이론값·계산식·나중에 채울 실험값 자리**는 \`data_table\`에 넣습니다.
- 계산 가능한 이론값은 수식만 쓰지 말고 최종 숫자를 문장에도 반복하세요. 예: "따라서 이론적 수득량은 2.609 g이다."
- Discussion/Conclusion/Questions 항목은 \`expected_discussion\`에 원문 순서대로 1:1 대응시킵니다. 평가자가 체크리스트로 보는 항목이므로 이름을 바꾸거나 합치지 마세요.
- 이론·메커니즘·재결정·수득률 설명에도 참고문헌을 붙일 수 있게 \`references\`에는 시약 물성 출처뿐 아니라 교과서/신뢰 가능한 웹 출처도 포함하세요.
...
- 친핵성 아실 치환 메커니즘이 평가 포인트이면 \`figures_needed\`에 메커니즘 그림 1개를 넣고 theory.items에 \`{ "figure": N }\`을 배치하세요. Aspirin 합성에서는 protonated anhydride, phenolic -OH attack, tetrahedral intermediate, proton transfer, acetic acid leaving step이 보이게 설명하세요.
...
최종 출력은 단 하나의 \`\`\`json ... \`\`\` 코드 블록입니다.`,
  ...
  // docx-gen이 minimal vs default 분기에 사용 (hwpx-gen은 default만 지원)
