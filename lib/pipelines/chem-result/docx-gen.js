...
function buildPurpose(content) {
  ...
  items.forEach((item, i) => {
    blocks.push(p(`(${i + 1}) ${item}`, { indent: { left: 360 } }));
  });
...
function buildTheory(content) {
  ...
  sections.forEach((sec, i) => {
    blocks.push(p(`${ka[i] || "?"}. ${sec.topic}`, { bold: true }));
    const items = Array.isArray(sec.items) ? sec.items : [];
    items.forEach((item, j) => {
      if (typeof item === "string") {
        blocks.push(p(`(${j + 1}) ${item}`, { indent: { left: 360 } }));
      }
...
function buildApparatusAndChemicals(content) {
  ...
  chems.forEach((c, i) => {
    const head = `${c.iupac || c.name}(${c.formula || ""}, ${c.molar_mass || ""})`;
...
function buildProcedure(content) {
  ...
  procs.forEach((proc, i) => {
    blocks.push(p(`${ka[i] || "?"}. ${proc.title}`, { bold: true }));
...
function buildPhotoBlocks(exp, allPhotos, figCounter) {
  ...
  for (const idx of indices) {
    ...
    figCounter.value += 1;
    const captionText = `[그림 ${figCounter.value}] ${exp.photo_caption || exp.name}`;
...
function buildData(content, allPhotos, figCounter) {
  ...
  exps.forEach((exp) => {
    ...
    if (exp.table?.headers && Array.isArray(exp.table.rows)) {
  ...
  if (data.summary_table?.headers && Array.isArray(data.summary_table.rows)) {
  ...
  for (const chart of charts) {
    ...
    // 캡션 — 마커 처리 위해 p() 사용
    const captionText = `[그래프 ${chartIdx}] ${chart.title || ""}${chart.caption ? " — " + chart.caption : ""}`;
    blocks.push(
      p(captionText, {
        align: AlignmentType.CENTER,
        italic: true,
  ...
  if (exps.length === 0 && !data.summary) {
    blocks.push(p("(측정 데이터 부족 — 데이터 파일 첨부 필요)"));
  }
...
function buildDiscussion(content) {
  ...
  (d.analysis || []).forEach((a, i) => {
    blocks.push(p(`(${i + 1}) ${a}`, { indent: { left: 360 } }));
  });
  blocks.push(emptyP());

  blocks.push(p("나. 오차 분석 및 개선점", { bold: true }));
  (d.errors || []).forEach((e, i) => {
    blocks.push(p(`(${i + 1}) [오차] ${e}`, { indent: { left: 360 } }));
  });
  (d.improvements || []).forEach((imp, i) => {
    blocks.push(p(`(${i + 1}) [개선] ${imp}`, { indent: { left: 360 } }));
  });
  blocks.push(emptyP());
...
// ── Minimal 스타일 빌더들 ────────────────────────────────────────────────────
// 잘 만든 학생 보고서(02~07_결과_2414_정원석.pdf) 스타일.
...
function buildMinimalApparatus(content) {
  ...
  apps.forEach((a) => {
    ...
    blocks.push(p(`${a.name || ""}${detail}`, { indent: { left: 240 } }));
  });
  if (apps.length === 0) blocks.push(p("(기구 데이터 부족)"));
  ...
  chems.forEach((c) => {
    ...
    const head = headParts ? `${c.name || c.iupac || ""} (${headParts})` : `${c.name || c.iupac || ""}`;
    const details = [
...
function buildMinimalDiscussion(content) {
  ...
  if (analysis.length === 0 && !hasDiscussion) {
    ...
    analysis.forEach((a) => {
      blocks.push(p(a, { indent: { firstLine: 200 } }));
    });
  } else {
    // analysis 없으면 errors/improvements를 결론 본문으로
    [...errors, ...improvements].forEach((t) => {
      blocks.push(p(t, { indent: { firstLine: 200 } }));
    });
  }
  blocks.push(emptyP());
  ...
  if (analysis.length > 0 && hasDiscussion) {
    ...
    errors.forEach((e) => {
      blocks.push(p(e, { indent: { firstLine: 200 } }));
    });
    improvements.forEach((imp) => {
      blocks.push(p(imp, { indent: { firstLine: 200 } }));
    });
    blocks.push(emptyP());
  }
  return blocks;
