...
// 단순 텍스트 단락 (마커 처리 안 함 — Phase 2-5에서 parser.js 통합 예정)
      ...
      new TableCell({
        ...
        children: [
          new Paragraph({
            ...
            children: [
              new TextRun({ text: String(h), font: FONT, size: 20, bold: true }),
            ],
          }),
        ],
      }),
...
function buildData(content) {
  ...
  exps.forEach((exp) => {
    ...
    if (exp.photo_caption) {
      blocks.push(
        p(`[그림] ${exp.photo_caption} (Phase 2-5에서 사진 자동 삽입 예정)`, {
          indent: { left: 360 },
          italic: true,
        }),
      );
