...
async function generateReportContent({
  ...
  if (dataBuffer) {
    ...
    if (["png", "jpg", "jpeg"].includes(dataExt)) {
    ...
    } else if (dataExt === "txt" || dataExt === "csv") {
      // 텍스트는 user message에 직접 포함
      const text = dataBuffer.toString("utf8").slice(0, 50000); // 너무 길면 자름
      content.push({
        type: "text",
      ...
      // xlsx 등은 Phase 2-3에서 추가
      attachmentSummary.push(
        `데이터 ${dataExt} — 파싱 미구현 (Phase 2-3 예정), 무시됨`,
      );
    }
  }
