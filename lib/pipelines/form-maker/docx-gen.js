...
function renderHeading(blk) {
  ...
  return [
    new Paragraph({
      spacing: { before: st.before, after: st.after, line: 312 },
...
// ── figure ──────────────────────────────────────────────────────────────────
function photoDimensions(buffer, targetWidth) {
  try {
...
function renderFigure(blk, ctx, width = TABLE_WIDTH_TWIP) {
  ...
  if (selected.length) {
    for (const photo of selected) {
      const dim = photoDimensions(photo.buffer, PHOTO_TARGET_WIDTH_PX);
      out.push(
        new Paragraph({
...
function buildTitle(content) {
  ...
  out.push(
    new Paragraph({
      ...
      children: parseRichText(String(content.title || "문서"), {
        font: currentFont(),
        size,
        bold: true,
        allowHighlights: allowHighlights(),
