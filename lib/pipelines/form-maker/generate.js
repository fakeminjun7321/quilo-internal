...
async function generateReportContent({
  ...
  // 원문 2단 레이아웃 vs 정리본. auto/clean → 정리본(전체폭 linearize, 안정),
  // layout → 한글 신문식 2단(원문 2단에 가깝게, 렌더러가 colCount=2 적용).
  const LAYOUT_MODE = String(layoutMode).toLowerCase() === "layout" ? "layout" : "clean";
...
최종 출력은 단 하나의 \`\`\`json ... \`\`\` 코드 블록입니다.`,
  ...
  if (allPhotos.length > 0) {
    await resolveFigureCrops(parsed, allPhotos, { allowRedraw: figureRedraw, onProgress });
  }
...
function isFurnitureParagraph(b, runningHeader) {
  ...
  // ② 저작권 / 무단전재 / 꼬리말 마커 (짧은 한 줄일 때만 — 본문에 들어간 인용은 보존)
  if (text.length <= 200 && /저작권|무단\s*전재|무단\s*복제|ⓒ|©|copyright/i.test(text)) return true;
  // ③ 머리말 재현: title(runningHeader)과 거의 동일한 짧은 단락
  if (runningHeader && text.length <= 120) {
    const norm = (s) => s.replace(/\s+/g, " ").replace(/[“”"'()[\]]/g, "").trim().toLowerCase();
    if (norm(text) === norm(runningHeader)) return true;
  }
  return false;
}
...
//  - furniture 단락 → 제거
function sanitizeLayout(blocks, { runningHeader = "", stats = { flattened: 0, dropped: 0 } } = {}) {
  if (!Array.isArray(blocks)) return [];
  const out = [];
  for (const b of blocks) {
    ...
    if (blockType(b) === "columns" && Array.isArray(b.columns)) {
      const cleanedCols = b.columns.map((col) =>
        sanitizeLayout(colToBlocks(col), { runningHeader, stats }),
      );
      const tall = cleanedCols.some((col) => columnIsTall(col));
      if (tall) {
        stats.flattened++;
...
async function resolveFigureCrops(parsed, allPhotos, { allowRedraw = false, onProgress = () => {} } = {}) {
  const figures = [];
  ...
  for (const fig of figures) {
    ...
    try {
      ...
      if (wantRedraw) {
        try {
          buf = await editImage(buf, FIGURE_REDRAW_PROMPT);
          redrawn++;
        } catch (e) {
...
module.exports = { generateReportContent, resolveFigureCrops, sanitizeLayout, normalizeModelLatex };
