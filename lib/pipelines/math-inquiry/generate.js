...
최종 출력은 단 하나의 \`\`\`json ... \`\`\` 코드 블록입니다.`,
  ...
  // ── Stream (+ 끊김 시 이어쓰기) ─────────────────────────────────────────────
  // Render 배포 환경에서는 server→Anthropic 연결이 ~200초쯤에 끊겨 장문(A4 4~8쪽)
  // 생성이 "Premature close"로 실패한다(로컬 직결은 300초+ 정상 — 즉 SDK/agent 가
  // 아니라 호스팅 네트워크 경로의 컷오프). 매 연결이 ~200초에 죽으므로 처음부터
  // 재시도는 또 같은 벽에 막혀 영원히 못 끝낸다. 유일하게 통하는 방법은 지금까지
  // 받은 텍스트를 assistant prefill 로 넣어 "이어서" 생성하는 것 — 각 구간이 벽보다
  // 짧으면 끝까지 도달한다. max_tokens 로 잘린 경우도 동일하게 이어붙인다.
  const startedAt = Date.now();
  const elapsed = () => Math.floor((Date.now() - startedAt) / 1000);
  let charCount = 0;
  let finalText = "";
  let cost = null;
  let usageAgg = null; // 세그먼트별 usage 누적(끊긴 구간은 usage 가 없어 미집계)
  ...
  const MAX_SEGMENTS = 6; // 이어쓰기 최대 구간 수(무한 루프 방지)
  let emptyDrops = 0; // 텍스트 0자에서 끊긴 횟수(순수 일시 오류)

  try {
    for (let seg = 1; seg <= MAX_SEGMENTS; seg++) {
      // 이어쓰기: 지금까지 받은 텍스트를 assistant turn 으로 prefill.
      // (assistant content 는 끝에 공백이 있으면 API 가 거부하므로 trailing 공백 제거 —
      //  JSON 출력에서 토큰 사이 공백은 무의미하므로 안전.)
      if (finalText) finalText = finalText.replace(/\s+$/, "");
      const messages = finalText
        ? [userMessage, { role: "assistant", content: finalText }]
        : [userMessage];

      let segText = "";
      let lastReportedChars = charCount;
      let lastEventAt = Date.now();
      let firstTokenSeen = finalText.length > 0; // 이어쓰기면 이미 시작된 상태로 본다
      let searchInFlight = false; // 진단용: 끊긴 순간 web_search 진행 중이었는지
      ...
      try {
        const stream = client.messages.stream({ ...baseParams, messages }, reqOptions);
        ...
        stream.on("streamEvent", (event) => {
          ...
          if (event.type === "content_block_start") {
            ...
            if (block?.type === "text") {
              if (!firstTokenSeen) {
                onProgress(`✍️ 보고서 작성 ${seg > 1 ? "이어서 " : ""}시작 (${elapsed()}초)`);
                firstTokenSeen = true;
              }
            } else if (
              ...
              webSearchTotal++;
              searchInFlight = true;
              lastSearchStartedAt = elapsed();
              onProgress(`🔍 선행연구 웹 검색 중... (${webSearchTotal}번째, ${elapsed()}초)`);
        ...
        if (finalMessage.stop_reason === "max_tokens" && seg < MAX_SEGMENTS) {
          continue; // 토큰 한도로 잘림 → 이어쓰기
        }
        break; // end_turn 등 정상 종료(또는 구간 소진) → 완료
      } catch (e) {
        if (signal && signal.aborted) throw e; // 사용자 중단·타임아웃은 재시도 안 함
        if (!isTransientStreamError(e)) throw e; // 진짜 오류는 그대로
        // 진단: 끊긴 순간의 상태를 기록(다음 Render 실패에서 원인 확정용). web_search
        // idle 가설을 검증하려면 searchInFlight / 마지막 검색 이후 경과가 핵심.
        const cause = (e && e.cause && (e.cause.code || e.cause.message)) || e.name || "?";
        const diag = `seg${seg} @${elapsed()}초 ${charCount}자 | 검색중=${searchInFlight}${searchInFlight ? `(시작 ${lastSearchStartedAt}초)` : ""} | cause=${cause} | msg=${String(e.message || e).slice(0, 80)}`;
        onProgress(`🔧 끊김 진단: ${diag}`);
        console.error(`[math-inquiry stream drop] ${diag}`);
        if (segText.length > 0) {
          // 일부라도 받았으면 살려서 이어쓰기(벽 우회).
          finalText += segText;
          if (seg >= MAX_SEGMENTS) break; // 마지막 구간 — 받은 데까지로 파싱 시도
          onProgress(
            `🔁 연결 끊김(${elapsed()}초, ${charCount}자) — 받은 내용에서 이어서 생성 (${seg}/${MAX_SEGMENTS})`,
          );
          continue;
        }
        // 응답 시작 전 끊김 = 순수 일시 오류 → 같은 지점에서 재시도.
        emptyDrops++;
        if (emptyDrops > 2 || seg >= MAX_SEGMENTS) throw e;
        onProgress(`🔁 연결 끊김(응답 시작 전) — 재시도 (${emptyDrops}/2)`);
        seg--; // 빈 끊김은 이어쓰기 구간으로 세지 않음
        continue;
      } finally {
    ...
    // 업로드한 PDF 정리(베스트에포트). 모든 구간이 끝났으니 더 이상 필요 없다.
