  ...
  async function streamWithContinuation(initialUserContent, { label, tools }) {
    ...
    for (let seg = 1; seg <= MAX_SEGMENTS; seg++) {
      ...
      try {
        ...
        if (finalMessage.stop_reason === "max_tokens" && seg < MAX_SEGMENTS) continue;
...
function extractJson(text) {
  // 모든 코드펜스를 먼저 제거한 뒤 가장 바깥 중괄호 구간을 잘라낸다. 이어쓰기로
  // 합쳐진 텍스트는 중간에 펜스가 다시 열리거나(```json) 닫히는(```) 경우가 있어,
  // 비탐욕 펜스 매칭(/```json...```/)을 쓰면 중간 펜스에서 잘려 JSON 이 깨진다.
  // 모델은 JSON 객체 바깥에만 산문을 두므로 first '{' ~ last '}' brace-slice 가 안전.
