...
function buildImageInstructions({ restoreOnly = false, chartRedraw = false, figures = null } = {}) {
  ...
  if (precise) {
    lines.push(
      "2) 그림 복원 — 원본 PDF에서 정확한 좌표로 잘라낸 아래 그림들을 내가 그대로 넣습니다. 그림을 말로 설명하거나 \\includegraphics 를 직접 쓰거나 그림 좌표 JSON 을 출력하지 마세요. 각 그림이 원래 나타난 자리(보통 해당 캡션 근처)에 `%%FIG:n%%` 한 줄만 정확히 넣으세요(한 그림당 한 번, 목록의 모든 n 에 대해 빠짐없이). 그림 캡션 문구는 본문에 평소 규칙대로 쓰되 마커는 반드시 남기세요.",
      figures
        .map(
          (f) =>
            `   - FIG ${f.n}: ${f.page}번째 이미지${f.caption ? ` · 캡션 "${String(f.caption).slice(0, 80)}"` : ""}`,
        )
        .join("\n"),
...
// figures + 원본 타일 버퍼 → {assets:[{name,buffer}], replace:{n: latex}, redrawn}.
...
// 마커 치환 + 모델이 본문에 마커를 빠뜨린 그림은 본문 끝에 보충한다 — 잘라낸 그림이
// 조용히 사라지는 일이 없게(그림 소실 방지의 최종 방어선).
function injectFiguresWithFallback(body, replace) {
  ...
  const missing = Object.keys(replace).filter((n) => !present.has(String(parseInt(n, 10))));
  for (const n of missing) out += `\n\n${replace[n]}`;
  return { body: out, appended: missing.length };
}
...
async function retypesetPdf({
  ...
  twoColumn = false,
  pageNumbers = true,
  ocrHint = null, // 스캔본에 심긴 '숨은' OCR 텍스트층(참고 힌트) — 판독 기준은 항상 이미지
  ...
  if (useChunks) {
    ...
    body = parts
      .map((p) => {
        ...
        const inj = injectFiguresWithFallback(p.parsed.body, replace);
    ...
    const textInstr = useFigures
      ...
      : TEXT_INSTR;
    // 스캔본에 숨은 OCR 텍스트층이 있으면 참고 힌트로 첨부(고유명사·숫자·코드 철자 보조).
    // 판독·번역 기준은 항상 이미지 — 힌트가 이미지와 다르면 이미지를 따르게 명시한다.
    const hintBlocks =
      useImages && ocrHint
        ? [
            {
              type: "text",
              text:
                "[참고: 이 스캔 PDF에 내장된 기존 OCR 텍스트층]\n" +
                "스캔 앱이 자동 생성한 텍스트라 오인식·누락·순서 뒤섞임이 있을 수 있습니다. " +
    ...
    if (visionFigures) {
      ...
      const inj = injectFiguresWithFallback(body, replace);
      body = inj.body;
      if (figureCount)
        onProgress(
          `🖼️ 원본 그림 ${figureCount}개 정밀 복원해 삽입` +
            (inj.appended ? ` (마커 누락 ${inj.appended}개는 본문 말미에 보충)` : ""),
        );
      ...
      if (scannedFigs.length) {
        ...
        figureCount = assets.length;
        const inj = injectFiguresWithFallback(body, cropped.replace);
        body = inj.body;
        if (figureCount)
          onProgress(
            `🖼️ 원본 그림 ${figureCount}개 복원해 삽입` +
  ...
  for (let attempt = 0; attempt <= MAX_REPAIR; attempt++) {
    ...
    const tex = buildTex({ body: balanceLatexEnvs(curBody), title, author, date, twoColumn, pageNumbers });
    try {
