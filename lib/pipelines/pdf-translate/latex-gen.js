...
function buildImageInstructions({ restoreOnly = false, chartRedraw = false } = {}) {
  const lines = [
    "위 이미지들은 한 문서를 위에서 아래 순서로 자른 페이지 조각입니다(1번부터 순서대로 제공; 경계가 약간 겹칠 수 있으니 중복된 문장은 한 번만 쓰세요).",
    "전체를 하나의 문서로 보고 다음을 수행하세요:",
    restoreOnly
      ? "1) 번역하지 마세요. 원문 언어 그대로(영어는 영어로) 본문을 충실히 전사하고, 수식을 정준형 LaTeX 로 복원해 재조판하세요. 문제 번호 등 원본 번호 체계를 유지하세요."
  ...
  if (useChunks) {
    ...
    const userContent = useImages
      ? [
          ...imageBlocks,
          ...hintBlocks,
          { type: "text", text: buildImageInstructions({ restoreOnly, chartRedraw }) },
        ]
