...
function figuresToAssets(figs) {
  ...
  const replace = {};
  const meta = {};
  // 동일 이미지가 여러 n 으로 중복 추출되면 하나의 에셋만 만들고 마커는 그 파일을 공유
  // (최종 dedupFigureImages 가 본문에서 중복 렌더를 한 번으로 정리한다).
  const seenHash = new Map(); // md5 → fig-name
  for (const f of figs || []) {
    if (!f || !f.buffer) continue;
    const h = crypto.createHash("md5").update(f.buffer).digest("hex");
    let name = seenHash.get(h);
    if (!name) {
      name = `fig-${f.n}.png`;
      seenHash.set(h, name);
...
// 최종 본문에서 같은 그림 파일(\includegraphics{X})이 두 번 이상 렌더되면 첫 번째만 남기고
// 나머지는 제거한다 — 모델이 같은 그림을 여러 자리에 넣었거나 동일 이미지가 중복 추출된
// 경우의 최종 안전장치(검증기가 잡던 '같은 FIGURE 2.45가 두 페이지에' 유형).
function dedupFigureImages(body) {
  const seen = new Set();
  return String(body).replace(
    /\\begin\{center\}\s*\n?\\includegraphics(?:\[[^\]]*\])?\{([^}]+)\}[\s\S]*?\\end\{center\}/g,
    (block, file) => {
      if (seen.has(file)) return ""; // 같은 그림 파일 재등장 → 제거
      seen.add(file);
      return block;
    },
  );
