...
const IMAGE_INSTRUCTIONS = [
  ...
  "   - box: 그 이미지 안에서 그림의 위치를 0~1 비율로 [좌, 상, 우, 하]. 이미지의 왼쪽 위가 (0,0), 오른쪽 아래가 (1,1). 그림(선·도형·축)만 감싸고 본문 글자는 최대한 제외하세요.",
...
async function retypesetPdf({
  ...
  const text = (message.content || [])
    ...
    .map((b) => b.text)
    .join("\n");
  const parsed = parseLatexOutput(text);
  ...
  if (useImages && Array.isArray(tiles) && tiles.length) {
    const figures = parseFiguresJson(text);
    if (figures.length) {
  ...
  // 남은(매칭 안 된) 마커 제거 — 본문에 raw 마커가 보이지 않게.
  body = injectFigures(body, {});
