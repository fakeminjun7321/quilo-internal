...
const parsedMaxMb = parseInt(process.env.MISTRAL_OCR_MAX_MB || "45", 10);
const MAX_MB = Number.isFinite(parsedMaxMb) && parsedMaxMb >= 1 ? parsedMaxMb : 45;
...
async function requestOcr(pdfBuffer, {
  ...
  for (let attempt = 1; ; attempt += 1) {
    let response;
    try {
      response = await fetchImpl(`${String(baseUrl).replace(/\/$/, "")}/ocr`, {
        method: "POST",
        headers: {
    ...
    if (retryable.has(status) && attempt < maxAttempts && !signal?.aborted) {
      await sleepImpl(backoffMs(attempt));
      continue;
    }
  ...
  if (status < 200 || status >= 300) {
    fail("OCR_PROVIDER_HTTP_ERROR", `Mistral OCR returned HTTP ${status}.`, {
      status,
      response_sha256: sha256Hex(raw),
      response_bytes: Buffer.byteLength(raw),
      request_id: responseRequestId(null, headers) || null,
    });
  }
  let json;
  try {
...
function assertStrictInput(pdfBuffer, { apiKey, strict }) {
  ...
  if (sizeMB > MAX_MB) {
    if (strict) {
      fail("OCR_INPUT_TOO_LARGE", "Strict OCR input exceeds the inline provider limit.", {
...
function validateVisualRenderAttestationShape(proof, inputDigest, inputCommitment) {
  assertExactObjectKeys(
    proof,
    [
      "schema_version",
...
async function ocrPdfToPageTexts(pdfBuffer, options = {}) {
  ...
  const { json } = await requestOcr(pdfBuffer, { ...options, apiKey });
