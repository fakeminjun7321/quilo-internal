...
const OCR_VISUAL_RENDER_ATTESTATION_SCHEMA_VERSION = 1;
...
function visualRenderAttestationPayload(inputCommitment, inputDigest) {
...
function createVisualRenderAttestation({
  ...
  const payload = visualRenderAttestationPayload(inputCommitment, inputDigest);
  return {
    ...payload,
...
function validateVisualRenderAttestation(visualAdjudication, { sourcePdf = null } = {}) {
  ...
  const payload = visualRenderAttestationPayload(commitment, inputDigest);
  ...
  const wrappedVisual = typeof visualAdjudicator === "function"
    ? async (summary, evidence) => {
        ...
        const renderedPages = validateRenderedAdjudicationPages(
          await renderPageProvider(pdfBuffer, requested, { signal }),
          requested,
          sourcePages,
        );
        ...
        const renderAttestation = createVisualRenderAttestation({
          sourcePdf: pdfBuffer,
          sourcePages,
          renderedPages,
        ...
        const result = await visualAdjudicator({
          ...
          sourcePages,
          renderedPages,
          adjudicationInput: binding.input,
          inputDigest: binding.inputDigest,
          signal,
        });
        if (!result || result.input_digest !== binding.inputDigest) {
          fail(
            "OCR_VISUAL_ADJUDICATION_INPUT_MISMATCH",
            "Visual adjudication result is not bound to the rendered tile input.",
            { expected_input_digest: binding.inputDigest },
          );
        }
