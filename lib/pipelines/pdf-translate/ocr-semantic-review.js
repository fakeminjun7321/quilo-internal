...
function partitionVisualPageIndices(pageCount) {
  const batchPages = Math.max(
    1,
    Math.min(8, Number.parseInt(process.env.PDF_OCR_VISUAL_BATCH_PAGES || "1", 10) || 1),
  );
...
async function buildVisualReview({
  ...
  let provider = null;
  let model = null;
  for (const indices of partitionVisualPageIndices(pageCount)) {
...
function partitionSemanticSegments(segments) {
  const maxPages = Math.max(
    1,
    Math.min(20, Number.parseInt(process.env.PDF_OCR_SEMANTIC_BATCH_PAGES || "8", 10) || 8),
  );
...
async function judgeSemanticBatches({
  ...
  const batches = [];
  const items = [];
  for (const batchSegments of partitionSemanticSegments(segments)) {
