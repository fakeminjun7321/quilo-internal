...
function assertCompleteRender(
  ...
  const virtualContractPresent = !hasFontExpected || (
    ...
    Math.max(0, Number(value.page_expected) || 0) === fontExpected &&
    Math.max(0, Number(value.page_drawn) || 0) === fontExpected &&
    virtualExpected === expected - fontExpected &&
    ...
    Math.max(0, Number(value.metadata_replaced) || 0) ===
      Math.max(0, Number(value.metadata_expected) || 0) &&
    Math.max(0, Number(value.drawn) || 0) === expected
  );
  ...
  if (replaced !== expected) reasons.push(`문단 삽입 ${replaced}/${expected}`);
