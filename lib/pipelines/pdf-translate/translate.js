...
const MAX_PAGES = parseInt(process.env.PDF_TRANSLATE_MAX_PAGES || "700", 10);
...
async function translateBlocksWithRetries({
  ...
  await runBatches(buildBatches(sourceBlocks, batchChars));

  let pendingBlocks = sourceBlocks.filter((block) => !translations[String(block.id)]);
  for (const size of Array.from(retrySizes || [])) {
    ...
    pendingBlocks = sourceBlocks.filter((block) => !translations[String(block.id)]);
  }
  ...
  return { translations, usage: usageSum, rejectionReasons };
}
