...
// 묶음 동시 번역 수(상한). 순차로 하면 페이지 많을 때 타임아웃 → 동시 처리.
// 작은 PDF 는 묶음이 적으므로 실제로는 min(CONCURRENCY, 묶음수)만 띄운다(효율 상한
// — 아래 runBatches 의 Math.min). 즉 이 값은 '큰 문서일 때의 천장'일 뿐이다.
const CONCURRENCY = Math.max(
  1,
  parseInt(process.env.PDF_TRANSLATE_CONCURRENCY || "10", 10),
);
