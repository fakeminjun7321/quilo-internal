...
async function translateBatch(client, model, blocks, signal) {
  ...
  const message = await client.messages.create(
    {
      model,
      // 출력이 잘리면 JSON 파싱 실패로 묶음 전체가 누락된다. 한국어가 원문보다
      // 길 수 있어 넉넉히 둔다(실제 출력 토큰만 과금되므로 비용 영향 없음).
      max_tokens: 16000,
      system: [
