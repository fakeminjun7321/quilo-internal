...
function buildBatches(
  ...
  for (const block of blocks) {
    ...
    const unitChars = unit.reduce((sum, item) => sum + String(item?.text || "").length, 0);
    ...
    if (curChars >= maxChars || cur.length >= maxIds) {
      batches.push(cur);
      cur = [];
      curChars = 0;
      curPages = new Set();
...
function annotatePageContinuations(blocks) {
  ...
  for (const start of adjacent.keys()) {
    ...
    members.sort((a, b) =>
      (a.page - b.page) || (sourceOrder.get(a) - sourceOrder.get(b)),
    );
...
function pageContinuationIssue(members, translations) {
  ...
  for (let index = 0; index < targets.length - 1; index += 1) {
    ...
    const independentlyFinishedHead = /(?:했다|하였다|되었다|이었다|이다|한다|된다|있다|없다|였다|았다|었다|[가-힣]습니다)[.!?。？！]?$/.test(headValue);
    const firstTailClause = tailValue.split(/[.!?。？！]/, 1)[0].trim();
    const particleOnlyTail = /(?:을|를|이|가|은|는|에|의|과|와|도|만|까지|부터|에서|에게|께|보다|처럼|마다|조차|마저|로|으로)$/.test(firstTailClause);
