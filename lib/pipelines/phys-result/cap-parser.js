...
async function parseCap(buffer) {
  ...
  // 6) data/*.tmp 측정 데이터
  //
  // PASCO Capstone의 .tmp는 **단순 double 배열이 아님**. 12-byte record의
  // 반복으로 구성된다:
  //
  //     [4-byte uint32 LE: tag/valid flag] [8-byte double LE: 측정값]
  //
  // tag는 보통 0x01 (1 = 유효 샘플). 이 구조를 모르고 매 8바이트마다 double로
  // 읽으면 tag 바이트가 double의 일부로 잘못 해석되어 1e+99 같은 노이즈가
  // 데이터에 섞이고, 실제 측정값은 거의 못 골라낸다.
  //
  // 참고: 이 디코딩은 사용자가 보낸 실제 .cap (Centripetal Force) 파일에서
  // 역공학으로 확인되었다.
  const MAX_VALUES_PER_DATASET = 200000;
  const RECORD_SIZE = 12; // 4-byte tag + 8-byte double
  for (const path of Object.keys(zip.files)) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // tag != 1 → 유효 샘플 아님 (gap, deleted, paused 등). 건너뜀.
      if (tag !== 1) continue;
...
function summarizeForPrompt(parsed) {
  ...
  if (datasetKeys.length) {
    ...
    //    같은 measurement에 음수/양수 쌍이 존재하면 둘 중 절대값 큰 쪽
    //    하나만 대표로 사용. 이름 없는 dataset은 "(미상)" 그룹.
    const groups = {};
    for (const fname of datasetKeys) {
      const d = parsed.datasets[fname];
      const meta = (parsed.dataset_to_measurement || {})[fname] || {};
      // 시간축은 보통 측정값으로 안 쓰므로 dependent 만 묶음. independent는 약식.
      if (meta.dependent === false) continue;
      const key = (meta.measurement_name || "(미상)").trim();
      if (!groups[key]) groups[key] = [];
      groups[key].push({ fname, d, meta });
    }

    // 2) 의미 없는 짧은 dataset (count<5)이나 절대값 항상 0인 건 스킵
    const cleaned = {};
    ...
    lines.push(
      `## 측정 데이터 (${datasetKeys.length}개 raw dataset → ${Object.keys(cleaned).length}개 measurement로 정리)`,
    );
    lines.push(
    ...
    for (const [mname, arr] of Object.entries(cleaned)) {
      // run을 count 기준으로 사용자 측정 회차일 만한 것 우선 (10~500개 sample).
      // 너무 짧은 건 영점·노이즈, 너무 긴 건 백그라운드 시계열.
      ...
      shown.forEach((x, i) => {
        ...
        const samp = sampleEvenly(x.d.values, 5)
          .map((v) => v.toPrecision(4))
          .join(", ");
        lines.push(
          `  run ${i + 1}: n=${stats.n}, mean=${stats.mean.toPrecision(4)}, std=${stats.std.toPrecision(3)}, range=[${stats.min.toPrecision(4)}, ${stats.max.toPrecision(4)}], samples=[${samp}]`,
        );
      });
      lines.push("");
    }
