...
// ── zip-bomb 방어 상한 ───────────────────────────────────────────────────────
// file._data.uncompressedSize는 ZIP 중앙 디렉터리 헤더에서 그대로 읽은 값이라
// 공격자가 위조할 수 있다(작은 값을 선언해 놓고 실제로는 GB급으로 부풀리는
// stream). 따라서 헤더 값을 '통과 조건'으로 신뢰하지 않고, 실제 출력 바이트를
// 세면서 상한을 강제한다.
const CAP_DECOMP_MAX = 150 * 1024 * 1024; // 누적 압축 해제 상한
const CAP_ENTRY_MAX = 80 * 1024 * 1024; // 단일 항목 압축 해제 상한
const CAP_TOTAL_COMPRESSED_MAX = 64 * 1024 * 1024; // .cap 전체 압축본 상한
const CAP_MAX_COMPRESSION_RATIO = 200; // 선언 해제/압축 비율 상한(정상 .cap는 훨씬 낮음)

const CAP_BOMB_MESSAGE =
  ".cap 압축 해제 크기가 비정상적으로 큽니다(손상되었거나 비정상적인 파일일 수 있음).";

// 헤더를 믿지 않고 스트리밍으로 압축을 풀며, 출력 바이트가 상한을 넘으면 즉시
// 중단한다. JSZip 3.x의 file.nodeStream()을 사용해 전체 버퍼를 만들기 전에
// 폭탄을 차단한다.
function readCapEntryGuarded(file, maxBytes) {
  // 값싼 사전 검사(모두 헤더 기반이라 '통과' 판정엔 쓰지 않고 '거절'에만 쓴다).
  // - 선언된 해제 크기가 이미 ENTRY_MAX를 넘으면 거른다.
  // - 큰 항목이면서 선언된 해제/압축 비율이 비상식적으로 크면(=폭탄 신호) 거른다.
  //   작은 항목은 비율이 커도 실제 메모리 위협이 안 되므로 스트리밍 상한에 맡긴다.
  // 이 사전 검사를 통과해도 아래 스트리밍 루프가 실제 출력 바이트로 최종 강제한다.
  const data = file._data;
  if (data && typeof data.compressedSize === "number" && data.compressedSize > 0) {
    const declared = Number(data.uncompressedSize);
    if (Number.isFinite(declared)) {
      if (declared > maxBytes) {
        return Promise.reject(new Error(CAP_BOMB_MESSAGE));
      }
      if (
        declared > 1024 * 1024 &&
        declared / data.compressedSize > CAP_MAX_COMPRESSION_RATIO
      ) {
        return Promise.reject(new Error(CAP_BOMB_MESSAGE));
      }
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    stream.on("data", (chunk) => {
      ...
      total += chunk.length;
      if (total > maxBytes) {
        abort(new Error(CAP_BOMB_MESSAGE));
        return;
      }
      chunks.push(chunk);
    });
...
const MAX_USERDATA_VALUES_PER_DATASET = Math.max(
  1000,
  parseInt(process.env.CAP_USERDATA_MAX_VALUES || "20000", 10) || 20000,
);
const MAX_USERDATA_STRING_BYTES = Math.max(
  64 * 1024,
  parseInt(process.env.CAP_USERDATA_MAX_STRING_BYTES || String(4 * 1024 * 1024), 10),
);
...
// [type=2][length] 사용자 표 헤더가 이 위치에서 시작하는지 검사.
// length 허용 조건은 기존 시퀀셜 워크의 조건을 그대로 유지한다.
function isUserDataHeader(buf, off) {
  if (off + 8 > buf.length) return false;
  if (buf.readUInt32LE(off) !== 2) return false;
  const length = buf.readUInt32LE(off + 4);
  return length > 0 && length <= 4096 && length % 2 === 0 && off + 8 + length <= buf.length;
}

function decodeUserData(buf) {
  // 1) TLV 블록 워크. 블록 = [type=2][cellLen] 헤더 + 셀 데이터 영역.
  //    데이터 영역은 다음 유효 헤더 직전(없으면 버퍼 끝)까지이며, 두 sub-form을 포함한다.
  //     (B1) 가변 길이: 셀마다 헤더가 붙음 → 영역 = 셀 1개
  //     (B2) 고정 길이 반복: 헤더 1개 뒤에 셀들이 이어짐. 셀 사이에 2-byte NUL
  //          종결자가 끼는 변형이 있어(예: Exp. a = "1.47","1.47",NUL,"1.47",NUL,"1.47")
  //          NUL을 셀 경계로 삼고, NUL 없이 붙은 구간만 cellLen 폭으로 나눈다.
  //    (이전 구현은 헤더가 깨진 지점부터 남은 버퍼 전체를 cellLen 고정 폭으로 잘라
  //    NUL·zero 패딩·다음 블록 헤더 바이트까지 문자로 삼켰고, 그 결과 "1.47"이
  //    "1.4"/"47"로 쪼개지는 등 사용자 표 값이 파손됐다.)
  const rawStrings = [];
  let stringBytes = 0;
  let truncated = false;
  const pushRaw = (s, bytes) => {
    if (rawStrings.length >= MAX_USERDATA_VALUES_PER_DATASET) {
      truncated = true;
      return false;
    }
    stringBytes += bytes || Buffer.byteLength(String(s || ""), "utf16le");
    if (stringBytes > MAX_USERDATA_STRING_BYTES) {
      truncated = true;
      return false;
    }
    rawStrings.push(s);
  ...
  // 같은 내용의 (B2) 블록이 한 파일에 반복 저장되는 경우(저장 스냅샷 중복,
  // 예: Effect. Rot. m = "7.4"×4 블록이 두 번)가 있어, 셀 2개 이상 블록이 앞선
  // 블록과 완전히 같으면 한 번만 채택한다. 셀 1개짜리 블록((B1)에서 같은 값이
  // 여러 행에 반복되는 정상 케이스, 예: "55","55")은 중복 제거하지 않는다.
  const seenBlocks = new Set();

  let off = 0;
  while (off + 8 <= buf.length && !truncated) {
    ...
    if (!isUserDataHeader(buf, off)) break; // 알 수 없는 데이터이므로 방어적 중단
    const cellLen = length;
    const dataStart = off + 8;

    // 데이터 영역 끝 = 다음 유효 헤더 위치(2-byte 단위 탐색). 첫 셀은 항상
    // cellLen 바이트를 차지하므로 그 뒤부터 찾는다. ((B1)은 여기서 바로 다음
    // 헤더가 잡혀 영역이 셀 1개가 된다.)
    let dataEnd = buf.length;
    for (let p = dataStart + cellLen; p + 8 <= buf.length; p += 2) {
      if (isUserDataHeader(buf, p)) {
        dataEnd = p;
        break;
      }
    }

    // 영역을 NUL(2-byte) 경계 segment로 나누고, segment 내부는 cellLen 폭으로
    // 자른다. zero 패딩 구간은 segment가 생기지 않아 자연히 무시된다.
    const cells = [];
    let cellBytes = 0;
    let segStart = -1;
    const flushSeg = (endPos) => {
      if (segStart < 0) return;
      for (let s = segStart; s < endPos; s += cellLen) {
        const e = Math.min(s + cellLen, endPos);
        cellBytes += e - s;
        if (
          cells.length >= MAX_USERDATA_VALUES_PER_DATASET ||
          cellBytes > MAX_USERDATA_STRING_BYTES
        ) {
          truncated = true;
          break;
        }
        cells.push(buf.toString("utf16le", s, e));
      }
      segStart = -1;
    };
    let p = dataStart;
    for (; p + 2 <= dataEnd && !truncated; p += 2) {
    ...
    flushSeg(p); // 마지막 열린 segment (홀수 꼬리 byte는 UTF-16 문자가 아니므로 버림)
    off = dataEnd;

    if (cells.length >= 2) {
      const sig = cells.join("\u0001"); // U+0001: 셀 텍스트에 나올 수 없는 구분자
      if (seenBlocks.has(sig)) continue;
      seenBlocks.add(sig);
    }
    for (const cell of cells) {
      if (!pushRaw(cell, cell.length * 2)) break;
    }
  }
  ...
  for (const s of rawStrings) {
    ...
    if (cleaned.length >= MAX_USERDATA_VALUES_PER_DATASET) {
      truncated = true;
      break;
    }
  }
  ...
  for (const s of cleaned) {
    ...
    if (numbers.length + labels.length >= MAX_USERDATA_VALUES_PER_DATASET) {
      truncated = true;
      break;
    }
  }
  ...
  return {
    kind,
    raw_count: rawStrings.length + (truncated ? 1 : 0),
...
async function parseCap(buffer) {
  // zip-bomb 방어: 업로드 제한은 '압축본' 크기만 막고 해제 후 메모리는 못 막는다.
  // 전체 압축본 크기부터 상한을 두고, 항목별/누적은 실제 출력 바이트를 세며 막는다.
  if (buffer && buffer.length > CAP_TOTAL_COMPRESSED_MAX) {
    throw new Error(CAP_BOMB_MESSAGE);
  }

  const zip = await JSZip.loadAsync(buffer);

  // 항목별/누적 압축 해제 크기에 상한을 둔다. (data/*.tmp, images/*, main.xml 공통)
  // 헤더의 uncompressedSize는 신뢰하지 않고 readCapEntryGuarded가 실제 바이트를 센다.
  let decompTotal = 0;
  const DECOMP_MAX = CAP_DECOMP_MAX;
  const ENTRY_MAX = CAP_ENTRY_MAX;
  ...
  // main.xml도 가드 없이 통째로 풀면 zip-bomb에 노출된다(data/*.tmp·images/*와 동일 패턴).
  // 헤더가 아니라 실제 해제 바이트를 세며 ENTRY_MAX를 넘으면 스트림 도중에 중단한다.
  const mainXmlBuf = await readCapEntryGuarded(mainXmlFile, ENTRY_MAX);
  const xmlString = mainXmlBuf.toString("utf8");
  decompTotal += mainXmlBuf.length;
  ...
  // 보고서에 측정값이 들어가지 않았다.)
  // zip-bomb 방어: decompTotal/DECOMP_MAX/ENTRY_MAX는 parseCap 상단에서 선언되어
  // main.xml 추출 시점부터 누적 검사에 함께 반영된다.
  for (const path of Object.keys(zip.files)) {
    if (!path.startsWith("data/") || !path.endsWith(".tmp")) continue;
    const file = zip.files[path];
    if (file.dir) continue;
    // 헤더의 uncompressedSize를 믿지 않고 실제 해제 바이트를 세며 ENTRY_MAX를 강제한다.
    const buf = await readCapEntryGuarded(file, ENTRY_MAX);
