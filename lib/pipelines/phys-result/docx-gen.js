...
const TABLE_WIDTH_TWIP = convertMillimetersToTwip(170);
...
function buildPhotoBlocks(photoIndices, allPhotos, figCounter, captionPrefix) {
  ...
  const selected = indices.map((idx) => allPhotos[idx]).filter(Boolean);
  for (let start = 0; start < selected.length; start += 3) {
    ...
    const targetWidth = cols >= 3 ? 160 : cols === 2 ? 220 : PHOTO_TARGET_WIDTH_PX;
    const imageCells = [];
    const captionCells = [];
