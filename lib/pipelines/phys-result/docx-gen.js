...
function p(text, opts = {}) {
  // {{EQ:...}}/{{EQ-LATEX:...}} 마커는 유니코드 평문으로 변환해 raw 노출을 막는다.
...
function buildResults(content, allPhotos, figCounter, tableCounter) {
  ...
  blocks.push(...buildPhotoBlocks(setup.photo_indices, allPhotos, figCounter, "실험 장치", setup.photo_captions));
  blocks.push(emptyP());
  ...
  experiments.forEach((exp, i) => {
    ...
    // 사진
    blocks.push(...buildPhotoBlocks(exp.photo_indices, allPhotos, figCounter, exp.name || "", exp.photo_captions));
    blocks.push(emptyP());
  });
...
// 선택적 '추가 실험 및 의문점 해결' 섹션 (스키마의 additional_investigations[])
function buildAdditionalInvestigations(content) {
  const items = Array.isArray(content.additional_investigations)
    ? content.additional_investigations.filter((it) => it && (it.title || it.body))
    : [];
  if (items.length === 0) return [];
  const blocks = [p("▶ 추가 실험 및 의문점 해결", { bold: true, size: 22, spaceAfter: 60 })];
  items.forEach((it, i) => {
    if (it.title) blocks.push(p(`${["가", "나", "다", "라", "마"][i] || i + 1}. ${it.title}`, { bold: true, spaceAfter: 40 }));
    if (it.body) blocks.push(p(String(it.body), { spaceAfter: 80 }));
  });
  blocks.push(emptyP());
  return blocks;
}
...
async function generateDocxWithFont(content) {
  ...
  const children = [
    ...
    ...buildAdditionalInvestigations(content),
