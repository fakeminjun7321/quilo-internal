...
⚠️ **차트 이미지 내부 텍스트(전부 한국어)**: chart.title, chart.x_label, chart.y_label, chart.series[].label, chart.x_values, trendline/reference_line label은 **한국어로** 작성하세요(서버 렌더러가 NanumGothic 한글 폰트를 지원하므로 차트도 한글로 렌더됩니다). 단위·변수 기호(°C, mL, N, m/s², 1/T, ln P, mω²)는 그대로 둡니다. series.label에 Exp/Theory/Measured 같은 영어 대신 측정값/이론값/보정값처럼 한국어를 쓰세요. 본문·캡션도 한국어입니다.
...
⚠️ **언어 규칙(전부 한국어)**: report.title·experiments[].name·data_table.headers·caption·본문·분석은 물론, **차트 안 텍스트(chart의 title/x_label/y_label/x_values/series.label/trendline.label)도 한국어로** 작성하세요(서버 렌더러가 NanumGothic 한글 폰트 지원). 단위·변수 기호(°C, mL, N, m/s², 1/T, ln P, mω²)는 그대로. 표 헤더·실험명·제목·차트 라벨을 영어로 두지 마세요.
