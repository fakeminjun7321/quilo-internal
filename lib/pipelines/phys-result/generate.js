...
PASCO Capstone (.cap) 파일이 서버에서 자동 파싱되어 텍스트로 제공됩니다. 이 정보와 학생이 업로드한 실험 사진을 바탕으로 보고서를 작성하세요.

⚠️ **반드시 5페이지 이내**, 학교 평가기준 65점 만점을 모두 노립니다 (3가지 평가 항목 모두 최고점).
...
1. .cap 파싱 결과(워크북 페이지, 센서, 측정 데이터, 텍스트)를 분석.
2. 실험 사진을 vision으로 직접 보고 어느 사진이 어느 단계에 적합한지 판단.
3. 실험 파트별로 표·차트·분석을 작성 (모든 파트에 표+차트 필수).
4. 결론에 이론 연결 + 정량적 오차 + 문제 해결 사례 모두 포함.
5. 5페이지 분량을 의식하며 압축.
6. JSON 출력.
 ...
 *
 * @param {Object} args
 * @param {Buffer} args.capBuffer       PASCO Capstone .cap 파일 (필수)
 * @param {string} args.capName         파일명
 * @param {Array<{buffer, name, mimetype}>} args.photos  실험 사진들
 * @param {Buffer|null} args.formBuffer 결과보고서 양식 PDF (선택)
 * @param {Buffer|null} args.rubricBuffer 평가기준 (PDF 또는 이미지, 선택)
 * @param {string} args.rubricName
  ...
  if (formBuffer) {
    content.push({
      ...
      source: {
        ...
        data: formBuffer.toString("base64"),
      },
    });
    attachmentSummary.push(`양식 PDF (${Math.round(formBuffer.length / 1024)}KB)`);
  }

  // 평가기준 (PDF 또는 이미지)
  if (rubricBuffer) {
    const ext = (rubricName.split(".").pop() || "").toLowerCase();
    if (ext === "pdf") {
      content.push({
        type: "document",
        source: {
      ...
      attachmentSummary.push(`평가기준 PDF`);
    } else if (["png", "jpg", "jpeg"].includes(ext)) {
      const mime = ext === "png" ? "image/png" : "image/jpeg";
      content.push({
        type: "image",
        source: { type: "base64", media_type: mime, data: rubricBuffer.toString("base64") },
      });
      attachmentSummary.push(`평가기준 이미지`);
    }
  }
