...
**STYLE_MODE: default** (학교 양식 + 65점 평가 최적화)
...
위 스킬 명세의 \`minimal\` 모드 섹션을 정확히 따르라. 핵심:
- 표지·1.1/1.2 자동 번호·65점 평가기준·PCEI 모두 무시
- **분량은 5페이지 이내** (학교 양식). default처럼 4.5~5p 거의 채우는 건 X. 학생 보고서 스타일이라 자연스럽게 쓰되 5페이지 안에 들어가야 함.
- JSON 스키마는 \`minimal\` 모드 전용: \`purpose\`, \`theory\`, \`apparatus\`, \`procedure\`, \`data\`, \`discussion\`, \`references\` 사용
- default 모드의 \`experiments\`/\`conclusion\`/\`experiment_setup\` 필드는 출력하지 말 것
- 가. 나. 다. 헤더 사용 X. (1) (2) (3) 또는 ① ② ③만.
- 이론적 배경: 본 실험에 직접 쓰이는 물리 이론·법칙·공식을 (1) (2) (3) 하위번호로 정의 (한국어 용어명만, 영어 병기 X)
- 실험 준비물: 한 문단 콤마 나열 (PASCO 모델번호 금지, 일반 한국어 명사로)
- PCEI 없음 (\`pcei: {}\` 빈 객체)`,
};
...
function buildSystemPrompt(style = "default") {
  ...
  const fivePageWarning = isMinimal
    ? ""
    : `
...
3. 모드에 맞는 JSON 스키마로 작성 (default vs minimal).
4. JSON 출력.
...
위 데이터를 바탕으로 분석·통계 계산·차트 생성을 수행하세요.`,
  ...
  const styleHints = isMinimalStyle
    ? `**스타일: minimal — 잘 만든 학생 보고서.** 평가기준 65점은 무시. **분량은 5페이지 이내** (학교 양식 강제). 자연스러운 학생 보고서 스타일이지만 5페이지 안에 들어가야 함. JSON 스키마는 \`purpose\`, \`theory\`, \`apparatus\`, \`procedure\`, \`data\`, \`discussion\`, \`references\` 사용. \`experiments[]\`/\`conclusion\`/\`experiment_setup\` 필드는 출력하지 말 것.`
...
최종 출력은 단 하나의 \`\`\`json ... \`\`\` 코드 블록입니다.`,
  ...
  if (Array.isArray(parsed.theory)) stats.push(`이론 ${parsed.theory.length}개`);
  if (Array.isArray(parsed.procedure)) stats.push(`과정 ${parsed.procedure.length}개`);
  if (Array.isArray(parsed.data?.experiments))
    stats.push(`측정 실험 ${parsed.data.experiments.length}개`);

  // 차트 카운트는 default와 minimal에서 위치가 다름
  let chartCount = 0;
  if (Array.isArray(parsed.experiments)) {
    for (const e of parsed.experiments) if (e.chart) chartCount++;
  }
  if (Array.isArray(parsed.data?.charts)) chartCount += parsed.data.charts.length;
  ...
  // 차트 렌더링 — default: experiments[i].chart / minimal: data.charts[]
  if (chartCount > 0) {
