...
- 표지·1.1/1.2 자동 번호·5페이지 강제·65점 평가기준 모두 무시
- 분량 7~9페이지, 자연스러운 학생 보고서 스타일
...
- 이론적 배경: 본 실험에 직접 쓰이는 물리 이론·법칙·공식 5~7개를 (1)~(5) 하위번호로 1단락씩 정의
...
위 데이터를 바탕으로 분석·통계 계산·차트 생성을 수행하세요.`,
  ...
  const styleHints = isMinimalStyle
    ? `**스타일: minimal — 잘 만든 학생 보고서.** 5페이지 강제·평가기준 무시. 7~9페이지 자연스러운 학생 보고서. JSON 스키마는 \`purpose\`, \`theory\`, \`apparatus\`, \`procedure\`, \`data\`, \`discussion\`, \`references\` 사용. \`experiments[]\`/\`conclusion\`/\`experiment_setup\` 필드는 출력하지 말 것.`
