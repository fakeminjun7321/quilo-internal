...
서버가 입력 데이터(.cap 파일 또는 엑셀)를 자동 파싱해서 텍스트로 제공합니다. 이 정보와 학생이 업로드한 실험 사진(있으면), 매뉴얼 PDF(있으면)를 바탕으로 보고서를 작성하세요.
...
3. **각 실험 파트에 (a) 표 + (b) 차트 + (c) 5~7문장 이상의 풍부한 분석** 필수 작성.
4. 분석 안에 \`가.\`, \`나.\` → \`(1)\`, \`(2)\` 세부 항목으로 구조화.
5. 1.1 실험 장치는 \`가. 장치 구성\` / \`나. 절차\` 등으로 단락 분리.
6. 결론에 이론 연결 + 정량적 오차 + 문제 해결 사례 모두 포함.
7. **5페이지 거의 채우기** — 분량이 부족하면 분석 단락을 더 풍부하게.
8. JSON 출력.
...
1. dataset 파일명(Z_*.tmp)은 의미 없습니다. **measurement 이름과 통계만 보고 판단**하세요.

2. 같은 measurement에 여러 run이 있는 이유: 사용자가 한 .cap 파일 안에서 여러 조건(예: 질량 5g, 10g, 15g, ...)을 순차 측정했기 때문입니다. **각 run = 한 측정 조건**.

3. 일부 run은 영점 측정·연습·실패 시도일 수 있습니다 (mean이 거의 0이거나 std가 비정상으로 큼). **사용자 보고서 양식상 의미 있는 run만 골라** 표에 사용하세요.

4. 측정값은 sensor 방향성으로 음수일 수 있습니다 (Force (Negative) 등). 보고서 표에는 **절대값**을 사용하세요.

5. 평균(F_측정, v_avg 등)은 제공된 mean 값을 그대로 쓰면 됩니다 — first 5개 같은 임의 sample로 평균 추정하지 마세요.

6. 실험 페이지 텍스트 콘텐츠 (Procedure m, Analysis v 등)에 사용자가 적은 조건과 결과 단서가 있을 수 있습니다. 거기서 어떤 run이 어떤 조건인지 추론하세요.

7. 보고서에 들어가야 할 표는 보통 5~6개 측정값 (예: 질량 변화 6단계). 각 단계의 mean(force) + mean(speed) 같이 두 measurement의 같은 run index를 매칭해 행으로 묶으세요.

8. 부호와 단위, 유효숫자(보통 3~4자리)에 주의하세요.`,
    });
...
⚠️ **5페이지 이내**, **65점 만점 평가** (① 표/그래프 + 경향성 ② 이론연결 + 오차분석 ③ 문제 인식·해결) 모두 노립니다.

각 실험 파트마다 표 + 차트 spec(서버가 PNG 렌더) + 분석을 반드시 포함하세요.
