...
const MAX_CAP_EMBEDDED_IMAGES = parseInt(
  process.env.PHYS_CAP_EMBEDDED_IMAGE_LIMIT || "8",
  10,
);
...
function scaledInertiaCell(rawValue) {
  ...
  return formatPlainNumber(n * 1e4, { sig: 3 });
}
...
⚠️ **Capstone 내장 그래프 우선**: .cap 파일에서 추출된 그래프/피팅 화면 이미지가 vision 입력으로 제공되면, 같은 관계를 서버 chart로 새로 그리기보다 그 이미지를 우선 보고서 그림으로 사용하세요. 특히 Capstone 화면에 사용자가 선택한 구간, fit box, 피팅식, RMSE, 눈금/축이 보이면 그것을 실제 실험자가 정리한 분석 근거로 취급합니다. 단, 숫자는 이미지에서 명확히 읽히거나 파싱 텍스트/엑셀에 있는 값만 인용하고, 보이지 않는 값을 추정하지 마세요.
...
2. 이미지 자료가 있으면 vision으로 직접 보고 실험 사진인지, 데이터표 스크린샷인지, 그래프 스크린샷인지 구분한다. 표/그래프 스크린샷이면 읽히는 숫자·축·회귀식만 데이터로 사용한다. 이름이 "[CAP graph/image ...]"로 시작하는 이미지는 .cap 내부에서 추출된 Capstone 화면이므로, 그래프나 피팅 화면이면 해당 실험 파트의 \`photo_indices\`에 넣어 보고서에 우선 배치한다.
...
13. 샘플링 주기와 샘플링 주파수를 함께 쓸 때는 반드시 역수 관계를 맞추세요. 예를 들어 50 Hz는 20 ms 주기이며, 25 ms와 함께 쓰면 안 됩니다.`,
    ...
    const usableCapImages = (parsedCap.images || [])
      .filter((img) => Buffer.isBuffer(img.buffer) && img.buffer.length > 10 * 1024)
      .slice(0, Math.max(0, MAX_CAP_EMBEDDED_IMAGES));
    if (usableCapImages.length) {
      const firstIndex = photos.length;
      const capImageFiles = usableCapImages.map((img, idx) => ({
        buffer: img.buffer,
        name: `[CAP graph/image ${idx + 1}] ${img.filename}`,
        mimetype: img.mime,
      }));
      photos.push(...capImageFiles);
      content.push({
        type: "text",
        text: `=== Capstone 내장 그래프/이미지 사용 지침 ===

.cap 파일 안에서 추출한 이미지 ${usableCapImages.length}개를 vision 입력과 최종 보고서 삽입 후보로 추가했습니다.
이 이미지들의 photo index는 ${firstIndex}부터 ${firstIndex + usableCapImages.length - 1}까지입니다.

이 이미지가 Capstone에서 사용자가 직접 만든 그래프, 선택 구간, sine/linear fit, fit box, RMSE, 축/단위가 보이는 화면이면 같은 데이터를 서버 chart로 다시 그리기보다 해당 이미지를 \`experiment.photo_indices\`에 넣어 우선 사용하세요.
\`chart\` 필드는 cap 이미지에 없는 보조 비교 그래프가 꼭 필요할 때만 사용하세요.
캡스톤 그래프에서 보이는 피팅식·파라미터·RMSE는 이미지에서 명확히 읽히는 범위에서만 본문 분석에 인용하세요.`,
      });
      attachmentSummary.push(`.cap 내장 그래프/이미지 ${usableCapImages.length}장`);
      if ((parsedCap.images || []).length > usableCapImages.length) {
        onProgress(
          `🖼️ .cap 내장 이미지 ${usableCapImages.length}/${parsedCap.images.length}장만 Claude에 전송 (제한 ${MAX_CAP_EMBEDDED_IMAGES}장)`,
        );
      }
    }
  }
...
위 텍스트가 측정값 표, 계산 기록, 그래프 해석, 실험 메모 중 무엇인지 먼저 판단하세요. 숫자·단위·조건명이 있으면 보고서 표와 분석에 반영하되, 텍스트에 없는 값은 만들지 마세요. 사용자가 직접 정리한 측정값·제외 사유·평균 계산 기록이면 .cap 원자료보다 보고서 산출 근거로 우선 고려하세요.${parsedText.truncated ? "\n\n⚠️ 원본 텍스트가 길어 앞부분만 전달되었습니다. 누락 가능성을 검토하세요." : ""}`,
  ...
  for (const [imageIndex, photo] of photos.entries()) {
    ...
    const isCapImage = String(photo.name || "").startsWith("[CAP graph/image");
    content.push({
      type: "text",
      text: `=== 이미지 자료 #${imageIndex} (${photo.name}) ===
이 이미지는 실험 사진, 데이터 표 스크린샷, 그래프 스크린샷 중 하나일 수 있습니다. 표라면 행·열 제목과 숫자를 읽어 데이터로 사용하고, 그래프라면 축 이름·단위·추세·회귀식·표시된 값이 있는지 확인하세요. 이미지에서 읽히지 않는 숫자는 추정하지 마세요.${isCapImage ? "\n이 이미지는 .cap 내부에서 추출된 Capstone 화면입니다. 그래프/피팅 화면이면 같은 내용을 새 chart로 중복 생성하지 말고, 해당 실험 파트의 photo_indices에 이 번호를 넣어 보고서 그림으로 우선 사용하세요." : ""}`,
    });
    if (prepared.ok) {
