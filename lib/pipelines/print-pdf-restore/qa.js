...
async function tesseractText(buffer, bin, root, stem) {
  ...
  return runProcess(bin, [imagePath, "stdout", "-l", process.env.PRINT_RESTORE_TESSERACT_LANG || "kor+eng", "--psm", "6"], { timeout: 120000 });
}
      ...
      if (tesseract) {
        try {
          const [sourceText, outputText] = await Promise.all([
            tesseractText(sourcePhotos[i].buffer, tesseract, tessRoot, `src-${pageNumber}`),
            tesseractText(rendered.pages[i].buffer, tesseract, tessRoot, `out-${pageNumber}`),
          ]);
