...
function fixInlineLatex(s) {
  ...
  while (i < n) {
    ...
    if (c === "$") {
      ...
      if (j === -1) {
        // 닫는 $ 누락(모델이 $ 하나를 빠뜨림) → 나머지를 수식으로 보고 닫아 준다. 단순히
        //  $ 를 이스케이프하면 \approx·\frac 같은 수식명령이 텍스트모드에서 깨지므로, 차라리
        //  span 을 닫는다(한글은 Pretendard 로 수식모드에서도 렌더됨). 수식모드에서 주석/정렬로
        //  오작동하는 % # & 만 이스케이프.
        const rest = s.slice(i + close.length).replace(/(?<!\\)([%#&])/g, "\\$1");
    ...
    if (c === "\\" && (s[i + 1] === "[" || s[i + 1] === "(")) {
      const open = s.slice(i, i + 2); // "\[" 또는 "\("
      const closeTok = s[i + 1] === "[" ? "\\]" : "\\)";
      const j = s.indexOf(closeTok, i + 2);
      if (j === -1) {
        // 짝 없는 \[ / \( → 나머지를 수식으로 보고 닫는다(위 $ 와 같은 이유).
        const rest = s.slice(i + 2).replace(/(?<!\\)([%#&])/g, "\\$1");
        out += open + rest + closeTok;
        break;
      }
      out += s.slice(i, j + 2);
      i = j + 2;
  ...
  for (const b of arr) {
    if (typeof b === "string") {
      ...
      if (typeof b.equation === "string" && b.equation.trim()) {
        ...
        // 디스플레이 수식 안의 떠돌이 $(모델 실수)는 제거하고, 여러 줄(\\)은 aligned 로 감싼다.
        let eq = String(b.equation).replace(/\r\n/g, "\n").replace(/(?<!\\)\$/g, "");
        if (/\\\\/.test(eq) && !/\\begin\{/.test(eq)) {
