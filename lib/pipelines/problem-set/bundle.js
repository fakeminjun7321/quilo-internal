    ...
    if (
      ...
      try {
        ...
        try {
          ...
          onProgress(`⚠ 삽화 생성 실패(${i + 1}번째 해설): ${String(e2.message).slice(0, 90)}`);
        }
      }
      if (png && png.length) {
        ...
        assets.push({ name, buffer: png });
        entry.image = name;
        entry.imageCaption = a.image.caption || "개념 도식 (AI 생성)";
        imagesMade++;
        onProgress(`🖼 해설 삽화 ${imagesMade}장 생성(${PS_IMAGE_MODEL})`);
      }
    }
    if (entry.chart || entry.image) assetByIdx.set(i, entry);
  }

  // ── 3) 3종 PDF 컴파일 (각 PDF 독립 — 하나 실패해도 나머지는 출력) ─────────
  onProgress("📐 영어 문제지 조판 중...");
  const enPdf = await compileResilient(
    (safe) => buildWorksheetTex({ problems, lang: "en", nPerPage, title, figByIdx, safe }),
    assets,
    { signal, onProgress, label: "영어 문제지" },
  );
  onProgress("📐 한글 문제지 조판 중...");
  const koPdf = await compileResilient(
    (safe) => buildWorksheetTex({ problems, lang: "ko", nPerPage, title, figByIdx, safe }),
    assets,
    { signal, onProgress, label: "한글 문제지" },
  );
  onProgress("📐 해설지 조판 중...");
  const akPdf = await compileResilient(
    (safe) => buildAnswerTex({ answerKey, title, notes: content.notes, assetByIdx, safe }),
    assets,
    { signal, onProgress, label: "해설지" },
  );
