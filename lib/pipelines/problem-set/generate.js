...
// 병렬화 파라미터(멀티모달 호출은 무거워 translate(12)보다 보수적으로).
const CHUNK_PAGES = Math.max(
  1,
  parseInt(process.env.PROBLEMSET_CHUNK_PAGES || "4", 10) || 4,
);
const CONCURRENCY = Math.max(
  1,
  parseInt(process.env.PROBLEMSET_CONCURRENCY || process.env.CONCURRENCY || "3", 10) || 3,
);
...
=========== SKILL SPEC END ===========`;
  ...
  let answerKey = [];
  const solveNotes = [];
  const batches = chunk(problems, SOLVE_BATCH);

  if (problems.length <= SOLVE_BATCH) {
    // 적은 문제 — 기존 단일/교차검증 경로(검증됨).
    if (crossVerify) {
