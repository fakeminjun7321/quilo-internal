...
async function generateReadingLogBulk(input) {
  ...
  const contents = await mapLimit(books, GEN_CONCURRENCY, async (b, i) => {
    ...
    c.__teacher = b.teacher || ""; // 교사별 합본 그룹 키(generateBundle에서 사용)
    return c;
  });
...
async function generateBundle(content, ctx = {}) {
  ...
  // 엑셀에 '교사' 열이 있으면 같은 교사의 기록지들을 멀티섹션 hwpx 하나로 병합해
  // '제출용_교사별/' 폴더에 함께 담는다. 개별 파일은 그대로 유지(검토·수정용).
  ...
  for (const [teacher, idxs] of byTeacher) {
    ...
    try {
      ...
      const firstTitle = safeName(books[idxs[0]].book_title || "독서록");
      const extra = idxs.length > 1 ? ` 외${idxs.length - 1}권` : "";
      const gname = `제출용_교사별/${who ? who + "_" : ""}${firstTitle}${extra}(${safeName(teacher)}).hwpx`;
      zip.file(gname, merged);
    } catch (e) {
      // 합본 실패는 치명적이지 않다 — 개별 파일로 제출 가능하므로 경고만.
      onProgress(`⚠ ${teacher} 합본 병합 실패(개별 파일은 정상): ${e.message}`);
    }
  }
