...
async function generateReadingLogBulk(input) {
  ...
  // ⚠ '진행 중인 탐구·R&E' 유형은 금지 — 모델이 실제로 하지 않은 실험·연구 활동을
  //   구체적으로 지어내는 사고가 있었음(2026-07-03). 검증 부담 없는 평범한 상황만 쓴다.
  const REASON_SEEDS = [
    "수업 시간에 배운 내용에서 생긴 의문의 연장",
    "앞서 읽은 다른 책·영상에서 이어진 궁금증",
    "언론·유튜브에서 본 논쟁의 원전 확인",
    "친구·선생님의 추천을 반신반의하며",
    "도서관 서가에서 목차의 특정 장에 끌려서",
    "시험이 끝난 뒤에도 남아 있던 찜찜한 개념 하나",
  ];
  ...
  if (byTeacher.size) {
    ...
    for (const [, grp] of byTeacher) {
      ...
      try {
        ...
        const label = safeName(subjLabel) || safeName(books[idxs[0]].book_title || "독서록");
        const cnt = idxs.length > 1 ? ` ${idxs.length}권` : "";
        const fname = addUnique(`${who ? who + "_" : ""}${label}(${safeName(teacher)})${cnt}`);
