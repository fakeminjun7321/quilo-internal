...
async function generateReadingLogBulk(input) {
  ...
  const contents = await mapLimit(books, GEN_CONCURRENCY, async (b, i) => {
    ...
    // 엑셀 '교과명' 열이 있으면 책별로 학생부 기록영역을 확정한다:
    // 교과명(물리 등) → 과목별 독서기록(교과명 기입), '담임(교사)' → 공통 독서기록(○).
    const subjName = String(b.subjectName || "").trim();
    const isHomeroom = /담임/.test(subjName);
    ...
    c.__subjectName = subjName; // 합본 파일명용(예: 물리(박홍).hwpx)
    return c;
  });
