...
async function generateReportContent({
  ...
  async function attemptGenerate() {
      ...
      const resp = await client.messages.create(
        {
          ...
          // 문장 리듬 다양화(특히 bulk 간 표현 중복 완화). Opus 4.8은 temperature 를
          // deprecated 400 으로 거부하므로(ai-studio-core 참고) Sonnet 계열에만 적용.
          ...(/claude-sonnet/.test(MODEL || "") ? { temperature: 0.8 } : {}),
          system: loadSkill(),
    ...
    return { parsed: p, selectionReason: sr, contentSummary: cs, reflection: rf };
  }
  ...
  const { parsed, selectionReason, contentSummary, reflection } = attemptResult;
