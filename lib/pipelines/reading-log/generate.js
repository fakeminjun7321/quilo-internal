...
async function generateReportContent({
  ...
  const selectionReason = clampSection(toParas(parsed.selection_reason));
  const contentSummary = clampSection(toParas(parsed.content_summary));
  const reflection = clampSection(toParas(parsed.reflection));
  ...
  const canonicalTitle =
    String(parsed.book_title || "").trim().slice(0, 200) || title;
