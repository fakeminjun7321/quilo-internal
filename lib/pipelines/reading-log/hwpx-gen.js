// 독서활동 기록지(독서록) HWPX 생성: Node에서 Python hwpx-gen.py 를 spawn 하고
// JSON payload 를 stdin 으로 넘긴다. (phys-inquiry/hwpx-gen.js 와 동일 골격)
...
function generateHwpx(content, { signal } = {}) {
  return new Promise((resolve, reject) => {
    ...
    proc.on("error", (err) => {
      reject(
        new Error(
          `reading-log hwpx-gen.py 실행 실패: ${err.message} (PYTHON_BIN=${PYTHON})`,
        ),
      );
    });
    ...
    proc.on("close", (code) => {
      ...
      if (code !== 0) {
        ...
        reject(
          new Error(
            `reading-log hwpx-gen.py 종료 코드 ${code}\n${stderr.slice(0, 1000)}`,
          ),
        );
        return;
      }
    ...
    proc.stdin.on("error", (err) => {
      reject(
        new Error(
          `reading-log hwpx-gen.py stdin 쓰기 실패(EPIPE 등): ${err.message}`,
        ),
      );
    });
