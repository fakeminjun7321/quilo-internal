...
function normalizeEntry(raw, { allowedKinds, pageText, pageNumbers, pageTexts, fallbackSection }) {
  ...
  const matchingPages = Array.isArray(pageTexts)
    ? pageTexts.filter((page) => termExistsInSource(term, page && page.text)).map((page) => page.number)
    : pageNumbers;
...
function unitPrompt(pages, input, unitNumber) {
  const source = pages.map((page) => `[[PDF PAGE ${page.number}]]\n${page.text}`).join("\n\n");
  return `이 페이지 묶음에서 단어장 항목 ${input.termCount}개를 고르세요.

묶음 번호: ${unitNumber}
허용 종류: ${kindsInstruction(input)}
발음 표기: ${input.includePronunciation ? "작성" : "pronunciation은 빈 문자열"}
원문에 정확히 나타난 term만 선택하고 source_page를 정확히 적으세요.
...
async function generateUnit(pages, input, unitNumber, { model, signal }) {
  ...
  if (entries.length < Math.min(8, input.termCount)) {
    throw new Error(
      `PDF ${pageNumbers[0]}-${pageNumbers.at(-1)}쪽에서 출처가 확인된 어휘가 ${entries.length}개뿐입니다. 텍스트가 선명한 PDF인지 확인하거나 범위를 바꿔 주세요.`,
    );
  }
  return {
    number: unitNumber,
    title_en: cleanText(response.data.title_en, 120) || `Pages ${pageNumbers[0]}-${pageNumbers.at(-1)}`,
    title_ko: cleanText(response.data.title_ko, 120) || `교재 ${pageNumbers[0]}-${pageNumbers.at(-1)}쪽`,
...
async function generateReportContent({
  ...
  onProgress("📖 PDF 텍스트와 페이지 구조를 읽는 중...");
  const extracted = await extractPdfText(sourcePdf.buffer, {
    pageRange,
    maxPages: MAX_PAGES,
    signal,
  });
  const textPages = extracted.pages.filter((page) => page.text && page.text.length >= 40);
  if (!textPages.length) {
    throw new Error("선택한 범위에서 읽을 수 있는 영어 텍스트를 찾지 못했습니다. 스캔 PDF는 먼저 OCR해 주세요.");
  }
  const skippedPages = extracted.pages.filter((page) => !page.text || page.text.length < 40).map((page) => page.number);
  const units = chunkPages(textPages, pagesPerUnit);
  onProgress(`✓ ${extracted.page_count}쪽 중 ${extracted.selected_pages.length}쪽 선택 · ${units.length}개 묶음`);
  let finished = 0;
  const generated = await mapLimit(
    units,
    CONCURRENCY,
    async (pages, index) => {
      onProgress(`🧠 묶음 ${index + 1}/${units.length} 어휘 선별 중 (PDF ${pages[0].number}-${pages.at(-1).number}쪽)`);
  ...
  if (skippedPages.length) {
    warnings.push(`텍스트가 거의 없는 PDF 페이지 ${skippedPages.slice(0, 12).join(", ")}${skippedPages.length > 12 ? " 외" : ""}는 어휘 선별에서 제외했습니다.`);
  }
  ...
  const resolvedTitle = cleanText(title, 160) || cleanText(extracted.title, 160) || sourcePdf.name.replace(/\.pdf$/i, "");
  const content = {
    title: resolvedTitle,
    book: {
      title: resolvedTitle,
      subtitle: "VOCABULARY",
      source_line: `${sourcePdf.name} · PDF ${extracted.selected_pages[0]}-${extracted.selected_pages.at(-1)}쪽`,
