...
const FEATURES = [
  ...
  feature("pdf-translate", "PDF 통번역", "translate", "일반 PDF·스캔본·수식 문서를 분석해 번역하거나 깨끗하게 재조판합니다.", "/translate.html", { kind: "workspace", status: "max", audience: "max", formats: ["pdf"] }),
  feature("cap-translate", "Capstone 번역", "translate", "PASCO Capstone 파일의 화면 텍스트를 번역한 파일을 만듭니다.", "/exam-prep.html", { kind: "pipeline", status: "paused", audience: "pro", formats: ["zip"] }),
  feature("create", "창작 스튜디오", "create", "대화형 미리보기와 체크포인트로 웹 아티팩트를 제작합니다.", "/create.html", { kind: "studio", status: "pro", audience: "pro" }),
  feature("vibe-coding", "바이브 코딩 생성기", "create", "아이디어 한 문장에서 프로젝트 구조와 구현 방향을 설계합니다.", "/vibe-coding.html", { kind: "studio", status: "pro", audience: "pro" }),
  feature("quilo-code", "Quilo Code", "create", "브라우저에서 코드를 작성하고 AI 도움을 받는 코딩 작업공간입니다.", "/editor.html", { kind: "studio", status: "pro", audience: "pro" }),
  feature("physics-studio", "고급 물리 문제 스튜디오", "study", "주제와 난이도에 맞춘 심화 물리 문제와 풀이를 만듭니다.", "/physics-studio.html", { kind: "studio", status: "pro", audience: "pro" }),
  feature("file-chat", "파일 챗봇", "study", "업로드한 파일을 바탕으로 질문하고 답변을 받습니다.", "/filechat.html", { kind: "studio", status: "pro", audience: "pro" }),
  ...
  feature("word-count", "글자수 세기", "translate", "텍스트의 글자수와 분량을 브라우저에서 계산합니다.", "/tools/word-count.html", { kind: "browser-tool", status: "active", audience: "public" }),
  feature("regression", "선형회귀·추세선", "translate", "데이터에서 회귀식과 결정계수, 그래프를 계산합니다.", "/tools/regression.html", { kind: "browser-tool", status: "active", audience: "public" }),
  feature("graph", "그래프 생성기", "translate", "엑셀·CSV 데이터로 산점도·선·막대그래프를 만듭니다.", "/tools/graph.html", { kind: "browser-tool", status: "active", audience: "public" }),
  feature("file-convert", "파일·PDF 변환기", "translate", "표·이미지·PDF를 브라우저에서 변환하고 편집합니다.", "/tools/convert.html", { kind: "browser-tool", status: "active", audience: "public" }),
  feature("equation", "LaTeX→한글 수식", "translate", "LaTeX 수식을 한글 수식 스크립트 또는 문서 객체로 변환합니다.", "/equation/", { kind: "browser-tool", status: "active", audience: "public" }),
  feature("community", "커뮤니티", "community", "건의·기능 요청·댓글과 운영 소통을 한곳에서 관리합니다.", "/community.html", { kind: "community", status: "active", audience: "member" }),
  feature("lab", "Quilo Lab", "community", "Quilo에 실제로 적용된 기술과 코드 설명을 공개합니다.", "/community.html?tab=lab", { kind: "knowledge", status: "active", audience: "public" }),
  ...
  feature("dropbox", "Dropbox 연동", "account", "완료 결과물을 사용자의 Dropbox에 저장합니다.", "/#settings", { kind: "integration", status: "active", audience: "member" }),
...
function feature(id, title, category, summary, path, options = {}) {
  return { id, title, category, summary, path, ...options };
}

function listFeatures({ category, status, audience, query } = {}) {
  let items = FEATURES;
  ...
  if (query) {
    ...
    items = items.filter((item) => [item.id, item.title, item.summary, item.category].join(" ").toLowerCase().includes(q));
  }
...
module.exports = { CATEGORIES, FEATURES, listFeatures, getFeature };
