...
// ── 예산 이미지 티어 (선택) ────────────────────────────────────────────────────
// 장식(decoration) 삽화처럼 품질 바가 낮은 그림은 FLUX.1 schnell 같은 저가 모델로
// ~10배 싸게 생성한다. IMAGE_BUDGET_API_KEY(또는 TOGETHER_API_KEY) 설정 시에만 활성.
// 미설정·실패 시 기존 gpt-image-1 경로로 폴백해 동작이 바뀌지 않는다.
// 라벨 많은 개념도(concept)는 텍스트 렌더 품질 때문에 계속 gpt-image-1 을 쓴다.
const IMAGE_BUDGET_BASE =
  process.env.IMAGE_BUDGET_API_BASE || "https://api.together.xyz/v1";
const IMAGE_BUDGET_MODEL =
  process.env.IMAGE_BUDGET_MODEL || "black-forest-labs/FLUX.1-schnell";
function budgetImageKey() {
  return process.env.IMAGE_BUDGET_API_KEY || process.env.TOGETHER_API_KEY || "";
}
function budgetImageAvailable() {
  return !!budgetImageKey();
}
// "WxH" → {width,height}. 예산 제공자는 size 문자열 대신 width/height 를 받는다.
function parseSize(size) {
  const m = /^(\d+)x(\d+)$/.exec(String(size || "1024x1024"));
  return m ? { width: +m[1], height: +m[2] } : { width: 1024, height: 1024 };
}
// 예산 이미지 제공자(Together 등 OpenAI 호환 images/generations) 호출 → PNG Buffer.
// 실패 시 throw(호출부가 gpt-image-1 로 폴백).
async function genImageBudget(prompt, { size, signal } = {}) {
  const key = budgetImageKey();
  if (!key) throw new Error("예산 이미지 키가 없습니다.");
  const { width, height } = parseSize(size);
  const resp = await fetch(`${IMAGE_BUDGET_BASE}/images/generations`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      model: IMAGE_BUDGET_MODEL,
      prompt: String(prompt || "").slice(0, 900),
      width,
      height,
      n: 1,
      steps: Number(process.env.IMAGE_BUDGET_STEPS) || 4,
  ...
  if (!resp.ok)
    throw new Error(`예산 이미지 ${resp.status}: ${raw.slice(0, 160)}`);
  let j;
  try {
    j = JSON.parse(raw);
  } catch {
    throw new Error("예산 이미지 응답을 해석할 수 없습니다.");
  }
  const d = j.data && j.data[0];
  if (d && d.b64_json) return Buffer.from(d.b64_json, "base64");
  if (d && d.url) {
    const img = await fetch(d.url, { signal });
    if (!img.ok) throw new Error(`예산 이미지 URL ${img.status}`);
    return Buffer.from(await img.arrayBuffer());
  }
  throw new Error("예산 이미지 응답이 비었습니다.");
}
...
async function callImageModel(prompt, size, model) {
...
// 단일 프롬프트 → PNG Buffer. model 미지정 시 기본 엔진(gpt-image-1).
// model 을 주면 그 모델로(예: "gpt-image-2"), 실패 시 호출자가 폴백한다.
async function genImage(prompt, { size, model } = {}) {
  const b64 = await callImageModel(prompt, size, model);
  return Buffer.from(b64, "base64");
}

// 입력 이미지(PNG/JPEG buffer) + 프롬프트 → 편집/재생성된 PNG Buffer (images/edits, 멀티파트).
// 문서 복원에서 "사진 속 그림을 깔끔히 다시 그림" 용. ⚠ 생성 모델이라 원본과 달라질 수 있다 —
// 호출부에서 옵트인 + 경고를 책임진다. 실패 시 throw(호출부가 원본 크롭으로 폴백).
async function editImage(imageBuffer, prompt, { size, model, signal } = {}) {
  const key = process.env.GPT_API_KEY || process.env.OPENAI_API_KEY;
  if (!key) throw new Error("이미지 생성 키(GPT_API_KEY)가 없습니다.");
  if (!Buffer.isBuffer(imageBuffer) || imageBuffer.length === 0) {
    throw new Error("편집할 이미지가 비었습니다.");
  }
  const form = new FormData();
  form.append("model", model || IMAGE_MODEL);
  form.append("image", new Blob([imageBuffer], { type: "image/png" }), "figure.png");
  form.append("prompt", String(prompt || "").slice(0, 900));
  form.append("size", size || "1024x1024");
  form.append("quality", "medium");
  const resp = await fetch(`${GPT_BASE}/images/edits`, {
    method: "POST",
  ...
  const raw = await resp.text();
  if (!resp.ok) throw new Error(`이미지 편집 ${resp.status}: ${raw.slice(0, 160)}`);
  const j = JSON.parse(raw);
  const b64 = j.data && j.data[0] && j.data[0].b64_json;
  if (!b64) throw new Error("이미지 편집 응답이 비었습니다.");
  return Buffer.from(b64, "base64");
}
...
async function renderFigures(
  ...
  for (let i = 0; i < list.length; i++) {
    const fig = list[i];
    try {
      // 장식 삽화 + 예산 제공자 설정 시 저가 모델 우선, 실패하면 기본 엔진으로 폴백.
      let buffer;
      if (fig.kind === "decoration" && budgetImageAvailable()) {
        try {
          buffer = await genImageBudget(buildSafePrompt(fig), {
            size: figSize(fig),
          });
        } catch (e) {
          onProgress(
            `⚠ 예산 이미지 실패 — 기본 엔진으로 폴백 (${String(e.message).slice(0, 60)})`,
          );
          buffer = await genImage(buildSafePrompt(fig), { size: figSize(fig) });
        }
      } else {
        buffer = await genImage(buildSafePrompt(fig), { size: figSize(fig) });
      }
