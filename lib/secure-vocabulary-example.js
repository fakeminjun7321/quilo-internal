...
const EXAMPLE = Object.freeze({
  ...
  pageCount: 290,
  byteLength: 4315053,
  sha256: "16ef323f282ac5b370fe5d87c0a060a636e2f79d270e7a90aa5265311ba0b5d5",
...
async function ensurePrivateBucket(supa) {
  const client = storageClient(supa);
  const { error: existingError } = await client.storage.getBucket(EXAMPLE.bucket);
  if (!existingError) return;
  const { error } = await client.storage.createBucket(EXAMPLE.bucket, {
    public: false,
...
async function bootstrapSource(supa, buffer) {
  validateSourcePdf(buffer);
  await ensurePrivateBucket(supa);
  const client = storageClient(supa);
  const { data: existing } = await client.storage.from(EXAMPLE.bucket).list("vocabulary", {
    search: path.posix.basename(EXAMPLE.objectPath),
    limit: 10,
  });
  if ((existing || []).some((item) => item.name === path.posix.basename(EXAMPLE.objectPath))) {
    return { created: false };
  }
  const { error } = await client.storage.from(EXAMPLE.bucket).upload(EXAMPLE.objectPath, buffer, {
...
function createSecureVocabularyExampleRouter({ requireAuth, getSessionUser, supa, sessionSecret, rootDir }) {
  ...
  // One-time, exact-hash bootstrap. It is removed immediately after the private
  // object is uploaded in production, so the final deployment exposes no write route.
  router.post(
    "/api/internal/bootstrap-vocabulary-example",
    express.raw({ type: "application/pdf", limit: "6mb" }),
    async (req, res) => {
      secureViewerHeaders(res, { content: true });
      try {
        const result = await bootstrapSource(supa, req.body);
        if (!result.created) return res.status(404).json({ error: "업로드 경로가 닫혔습니다." });
        return res.status(201).json({ ok: true, sha256: EXAMPLE.sha256, bytes: EXAMPLE.byteLength });
      } catch (error) {
        return res.status(400).json({ error: "등록된 예시 파일과 일치하지 않습니다." });
      }
    },
  );
