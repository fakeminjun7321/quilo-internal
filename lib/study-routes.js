...
- 그림의 핵심은 "무엇을 같은 프레임의 지금에서 재는가"입니다. 길이/시간/동시성 비교는 반드시 segments에 측정선으로 넣으세요.
- diagramType은 문제 성격에 맞춰 "length-contraction", "time-dilation", "simultaneity", "minkowski-clean" 중 하나를 쓰세요. 뮤온계 길이수축처럼 특별히 뮤온 관점이면 "muon-frame-length-contraction"을 써도 됩니다.
- segments는 그래프 안에 그릴 핵심 측정선입니다. 길이수축이면 수축 전 고유길이와 수축된 거리, 시간지연이면 고유시간과 관찰자 시간, 동시성이면 S의 동시선과 S'의 지금선을 넣으세요.
- 보정쌍곡선/불변쌍곡선은 1개 정도만 짧게 annotations에 넣으세요. 장식이 아니라 비교 기준을 보여주는 용도입니다.
...
function inferPromptKind(text) {
  ...
  const time = /시간지연|수명|고유시간|propertime|timedilation|lifetime|쌍둥이/.test(s);
...
function muonFrameLengthTemplate() {
  return {
    ...
    eta: Math.atanh(0.6),
    events: [
      { id: "O", label: "뮤온의 지금", x: 0, t: 0, color: "#0f9d6b" },
      { id: "G", label: "지면 지금", x: 8, t: 0, color: "#6b7280" },
      { id: "H", label: "생성 고도", x: -4.5, t: 5.7, color: "#c0562b" },
    ],
    worldlines: [
      { id: "muon", label: "뮤온", color: "#1d5fbf", points: [{ x: 0, t: -1.2 }, { x: 0, t: 7.2 }] },
      { id: "top_atmosphere", label: "생성 고도", color: "#2a9d69", points: [{ x: -5.6, t: 7.2 }, { x: 2.2, t: -1.2 }] },
      { id: "ground", label: "지면", color: "#2a9d69", points: [{ x: 3.0, t: 7.2 }, { x: 10.6, t: -1.2 }] },
    ],
    annotations: [
      { label: "D0=10", x: 3.6, t: 3.9, color: "#c0562b" },
      { label: "D=8", x: 4, t: -0.75, color: "#047857" },
      { label: "보정쌍곡선", x: 5.4, t: 2.5, color: "#b45309" },
    ],
    explanation: "뮤온계에서는 뮤온이 정지하고 지면이 다가옵니다. 같은 t′=0에서 잰 지면까지의 거리는 D0=10이 아니라 D0/γ=8로 짧아집니다.",
    assumptions: ["속도가 명시되지 않아 beta=0.6, D0=10, D0/gamma=8을 도식용 값으로 사용"],
    warnings: [],
  };
}

function lengthContractionTemplate(description) {
  const isMuon = /뮤온|muon/i.test(description || "");
  const subject = isMuon ? "뮤온" : "움직이는 관찰자";
  const target = /지면|땅|ground|earth/i.test(description || "") ? "지면" : "물체 끝";
  return {
    diagramType: isMuon ? "muon-frame-length-contraction" : "length-contraction",
    title: isMuon ? "뮤온계에서 본 길이수축" : "움직이는 계에서 본 길이수축",
    beta: 0.6,
    eta: Math.atanh(0.6),
    events: [
      { id: "O", label: `${subject}의 지금`, x: 0, t: 0, color: "#0f9d6b" },
      { id: "A", label: "왼쪽 끝", x: 4, t: 0, color: "#0f9d6b" },
      { id: "B", label: target, x: 12, t: 0, color: "#6b7280" },
      { id: "P", label: "고유길이 기준", x: 2, t: 7, color: "#c0562b" },
    ],
    worldlines: [
      { id: "observer", label: subject, color: "#1d5fbf", points: [{ x: 3, t: -1 }, { x: 3, t: 8 }] },
      { id: "left_end", label: "가까운 끝", color: "#2a9d69", points: [{ x: 5, t: -1 }, { x: 1, t: 8 }] },
      { id: "right_end", label: target, color: "#2a9d69", points: [{ x: 12, t: -1 }, { x: 8, t: 8 }] },
    ],
    segments: [
      {
        label: "수축 전 고유길이 D0",
    ...
    annotations: [{ label: "보정쌍곡선", x: 8, t: 3.4, color: "#b45309" }],
    explanation:
      "움직이는 계에서는 같은 t′=0에서 양끝을 재야 하므로 고유길이 D0보다 짧은 D=D0/γ가 보입니다.",
    assumptions: ["속도가 명시되지 않아 beta=0.6을 도식값으로 사용", "D0와 D는 비율 설명용 도식값"],
    warnings: [],
  };
}
...
function timeDilationTemplate() {
  return {
    ...
    events: [
      ...
      { id: "A", label: "움직이는 시계의 다음 틱", x: 6, t: 8, color: "#c0562b" },
      { id: "B", label: "정지계 같은 위치 시간", x: 0, t: 8, color: "#1d5fbf" },
    ],
    worldlines: [
      { id: "lab", label: "정지 관찰자", color: "#1d5fbf", points: [{ x: 0, t: 0 }, { x: 0, t: 9 }] },
      { id: "clock", label: "움직이는 시계", color: "#c75a2a", points: [{ x: 0, t: 0 }, { x: 6.6, t: 8.8 }] },
    ],
    segments: [
      {
        label: "고유시간 Δτ",
    ...
    annotations: [{ label: "불변쌍곡선", x: 4.6, t: 5.2, color: "#b45309" }],
    explanation:
      "움직이는 시계의 두 틱 사이 고유시간 Δτ는 짧고, 정지계에서 읽는 시간 간격은 Δt=γΔτ로 더 깁니다.",
    assumptions: ["속도가 명시되지 않아 beta=0.6을 도식값으로 사용"],
    warnings: [],
  };
}
...
function simultaneityTemplate() {
  return {
    ...
    eta: Math.atanh(0.55),
    events: [
      { id: "A", label: "왼쪽 사건", x: 3, t: 4, color: "#0f9d6b" },
      { id: "B", label: "오른쪽 사건", x: 10, t: 4, color: "#0f9d6b" },
      { id: "O", label: "관찰자", x: 6.5, t: 0, color: "#1d5fbf" },
    ],
    worldlines: [
      { id: "left", label: "왼쪽 위치", color: "#2a9d69", points: [{ x: 3, t: 0 }, { x: 3, t: 8 }] },
      { id: "right", label: "오른쪽 위치", color: "#2a9d69", points: [{ x: 10, t: 0 }, { x: 10, t: 8 }] },
      { id: "moving", label: "움직이는 관찰자", color: "#1d5fbf", points: [{ x: 6.5, t: 0 }, { x: 11, t: 8 }] },
    ],
    segments: [
      {
        label: "S에서 동시",
    ...
    annotations: [{ label: "동시성 상대성", x: 6.4, t: 5.2, color: "#b45309" }],
    explanation:
      "S에서 같은 시간의 두 사건도 움직이는 계 S′에서는 같은 '지금' 선이 기울어져 동시에 보이지 않을 수 있습니다.",
    assumptions: ["속도가 명시되지 않아 beta=0.55를 도식값으로 사용"],
    warnings: [],
  };
}
...
function inferTextTemplate(description) {
  ...
  if (kind === "length-contraction") return lengthContractionTemplate(description);
...
function decorateInferredSpec(spec, description) {
  ...
  if (
    out.diagramType &&
    out.diagramType !== "minkowski-clean" &&
    out.diagramType !== "muon-frame-length-contraction" &&
    Array.isArray(out.annotations) &&
    !out.annotations.some((a) => /쌍곡|불변|보정/.test(String(a && a.label ? a.label : "")))
  ) {
    out.annotations = out.annotations.concat([{ label: "불변쌍곡선", x: 4.5, t: 3.2, color: "#b45309" }]).slice(0, 3);
  }
  return out;
}
...
module.exports = function studyRouter(deps = {}) {
  ...
  r.post(
    ...
    async (req, res) => {
      ...
      const template = !file ? inferTextTemplate(description) : null;
      if (template) {
        const out = sanitizeSpec(template);
        out.model = "template";
      ...
      try {
        ...
        for (const model of candidates) {
          try {
            ...
            const out = decorateInferredSpec(sanitizeSpec(parsed), description);
            const info = modelInfo(model);
            out.model = model;
