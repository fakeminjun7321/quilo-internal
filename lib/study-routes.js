...
function sanitizeSpec(input) {
  ...
  return {
    diagramType:
      src.diagramType === "muon-frame-length-contraction"
...
module.exports = function studyRouter(deps = {}) {
  ...
  r.post(
    ...
    async (req, res) => {
      ...
      if (!file && isMuonFrameLengthPrompt(description)) {
        const out = sanitizeSpec(muonFrameLengthTemplate());
        out.model = "template";
        out.modelLabel = "뮤온 길이수축 템플릿";
        return res.json(out);
      }
