...
module.exports = function subscriptionRoutes({
  ...
  // 사용자: 프리미엄 입금 신청. body = { depositorName? }
  // 입금 후 신청 → 관리자가 입금 확인하고 승인하면 즉시 프리미엄 활성.
  router.post("/request", requireAuth, async (req, res) => {
    ...
    try {
      if (u.isAdmin || (await supa.getActiveBackgroundSub(u.id))) {
        return res
          .status(409)
          .json({ error: "이미 프리미엄이 활성화되어 있습니다." });
      }
    } catch (_) {}
    try {
      ...
      if (to) {
        mailer
          .sendEmail({
            to,
            subject: `[Quilo] 프리미엄 입금 신청 — ${u.name || u.id}`,
            text: `사용자 ${u.name || u.id} 가 프리미엄(백그라운드 실행)을 신청했습니다.\n입금자명: ${depositorName || "(미입력)"}\n\n관리자 페이지 → 🌙 백그라운드 탭 '입금 신청 대기'에서 입금 확인 후 승인하세요.`,
          })
          .catch(() => {});
      }
  ...
  // 관리자: 입금 신청 승인 → 즉시 프리미엄 부여(활성이면 만료일에서 연장).
  ...
  router.post("/requests/:id/approve", requireAdmin, async (req, res) => {
    ...
    try {
      ...
      await supa.decidePremiumRequest(reqRow.id, {
        status: "approved",
      ...
      try {
        ...
        if (target && target.email) {
          ...
          mailer
            .sendEmail({
              to: target.email,
              subject: "[Quilo] 프리미엄이 활성화되었습니다 ✨",
              text: `프리미엄이 활성화되어 이제 보고서를 백그라운드로 실행할 수 있습니다(제출 후 탭을 닫아도 됩니다).\n${base}`,
            })
            .catch(() => {});
        }
      } catch (_) {}
