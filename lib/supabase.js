 ...
 * 기존 비밀번호는 기본적으로 보존한다. 운영 복구 시에만 ADMIN_SYNC_PASSWORD=1을
 * 일시적으로 켜 환경변수 비밀번호를 다시 해시해 동기화한다.
...
async function ensureAdminFromEnv() {
  ...
  // ADMIN_NAME은 로그인 화면의 아이디로 사용되는 경우가 많다. 표시 이름만 조회하면
  // username과 name이 다른 기존 관리자를 못 찾아 신규 생성/복구가 실패할 수 있다.
  let admin = await findUserByUsername(name);
  if (!admin) admin = await findUserByName(name);
  if (admin) {
    const passwordMatches = verifyPassword(password, admin.password_hash);
    const syncPassword = process.env.ADMIN_SYNC_PASSWORD === "1";
    if (!admin.is_admin) {
      const allowExisting =
        process.env.ADMIN_ALLOW_EXISTING_PROMOTION === "1";
      // 일반 사용자 승격은 기존 비밀번호 일치 또는 명시적인 비밀번호 동기화가
      // 추가로 필요하다. ADMIN_SYNC_PASSWORD 하나만으로는 승격할 수 없다.
      if (!allowExisting || (!passwordMatches && !syncPassword)) {
        throw new Error(
          "ADMIN_NAME과 같은 일반 사용자 계정이 이미 있어 관리자 승격을 중단했습니다. " +
            "계정을 직접 확인하거나 ADMIN_ALLOW_EXISTING_PROMOTION=1 및 일치하는 ADMIN_PASSWORD를 설정하세요. " +
            "비밀번호가 유실된 운영 복구에서는 두 플래그와 ADMIN_SYNC_PASSWORD=1을 함께 사용하세요.",
        );
      }
      admin = await updateUser(admin.id, {
        isAdmin: true,
        ...(syncPassword && !passwordMatches ? { password } : {}),
      });
    } else if (syncPassword && !passwordMatches) {
      admin = await updateUser(admin.id, { password });
    }
    return admin;
  }
