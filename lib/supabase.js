...
// (운영자가 본인 학교 이메일로 여러 테스트 계정을 인증하는 용도). env VERIFY_EXEMPT_EMAILS(쉼표구분)로 override.
// ⚠ 개인 이메일(PII)이 기본값에 있으니 공개 저장소로 내보낼 때 스크럽할 것.
...
async function verifyEmailToken(tokenHash) {
  ...
  return {
    ok: true,
    user: { ...user, email: multi ? "" : email, email_verified: true },
  };
}
...
async function reserveCredits(userId, amount) {
  ...
  const rpc = await c.rpc("reserve_credits", { p_user_id: userId, p_amount: amt });
  if (!rpc.error) {
    ...
    return { ok: true, newBalance: bal, atomic: true };
  }
  const rpcMsg = `${rpc.error.message || ""} ${rpc.error.code || ""} ${rpc.error.hint || ""}`;
  const fnMissing =
    /PGRST202|42883|could not find the function|does not exist/i.test(rpcMsg);
...
async function getReportJob(userId, jobId) {
  ...
  try {
    const { data, error } = await c
      .from("report_jobs")
      .select(
        "id, report_type, model, status, filename, file_id, error, progress, background, notify_email, created_at, updated_at",
      )
      .eq("id", String(jobId))
      .eq("user_id", userId)
      .maybeSingle();
