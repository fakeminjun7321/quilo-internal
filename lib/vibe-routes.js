...
module.exports = function vibeRouter(deps = {}) {
  ...
  r.get("/config", (_req, res) => {
    res.json({
      ...
      imageAvailable: imageKeyAvailable(),
  ...
  // 대화형 수정: 현재 설계 + 대화이력 + 새 요청 → { reply, plan(갱신본) }. 로그인 + 모델 크레딧 차감.
  r.post(
    "/refine",
    requireAuth,
  ...
  // 개념 이미지 생성(gpt-image-2 → 폴백 gpt-image-1). 로그인 + 이미지 1장당 IMAGE_CREDITS 차감.
  r.post("/image", requireAuth, async (req, res) => {
    ...
    let fresh = userInfo;
    try {
      if (typeof refreshSessionUser === "function") fresh = (await refreshSessionUser(req)) || userInfo;
    } catch {}
    ...
    if (chargeable) {
      try {
        ...
        if (have < IMAGE_CREDITS) {
          return res.status(402).json({ error: `🚫 크레딧 부족 (보유 ${have} / 필요 ${IMAGE_CREDITS}).` });
        }
      } catch (e) {
        console.error("[vibe-image] credit check:", e.message);
    ...
    let buffer;
    try {
      buffer = await genImage(buildVibeImagePrompt(prompt), { size: "1536x1024", model: IMAGE_MODEL });
    } catch (e1) {
      try {
        buffer = await genImage(buildVibeImagePrompt(prompt), { size: "1536x1024", model: IMAGE_MODEL_FALLBACK });
      } catch (e2) {
        console.error("[vibe-image] generate:", e1.message, "| fallback:", e2.message);
        return res.status(502).json({ error: "이미지 생성에 실패했습니다: " + (e2.message || e1.message) });
      }
    }

    // 성공 후 차감(실패해도 결과는 반환)
    let creditsCharged = 0, newBalance;
    if (chargeable) {
      try { const s = await supa.spendCredits(userInfo.id, IMAGE_CREDITS); creditsCharged = IMAGE_CREDITS; newBalance = s.newBalance; }
      catch (e) { console.error("[vibe-image] spend:", e.message); }
    }
    return res.json({
      ok: true,
