...
let currentStudentId = "";
      ...
      function applyAuth(loggedIn, d) {
        ...
        if (loggedIn && d) {
          ...
          if (d.isAdmin) {
            ...
            // 신규 제공자 모델(Gemini/Qwen) — 신규 AI 기능은 관리자 전용으로 먼저 노출.
      ...
      async function submitReport({ formEl, buttonEl, formData, busyText = "생성 중...", estimate = null }) {
        // ⚠ 중국 서버(Qwen 등) 모델 — 전송 전 사용자 동의를 명시적으로 받는다(서버도 403 재검증).
        try {
          const m = formData && formData.get ? String(formData.get("model") || "") : "";
          if (/^qwen/i.test(m) && formData.get("foreignConsent") !== "true") {
            const ok = window.confirm(
              "선택한 모델(Qwen)은 가격이 매우 저렴하고 성능이 좋지만, 입력하신 자료가 " +
              "중국 소재 서버(Alibaba)로 전송됩니다.\n\n" +
